...
The hgtran-ai update/upgrade/sync flow is unreliable and passive: engram bumps force a hgtran-ai release, TUI users get only a silent banner, "Upgrade + Sync" doesn't fully sync, the upgrade executor ignores the active channel, and every launch hits the GitHub API with no cooldown. This change makes updates reach users without a hgtran-ai release, prompts users at launch (Codex-style), and fixes the three reliability gaps — while staying informational-only (no forced-update gate).

## Scope

### In Scope
1. **Engram always-latest** (slice 1, DONE — branch `fix/engram-always-latest`, commit `36aee12`): un-pin engram-core + hgtran-engram; runtime fetch filters tags to `^v[0-9]+\.[0-9]+\.[0-9]+$`; unifies download with update-check source of truth; prerelease/rc tags stay invisible until a clean `vX.Y.Z` is cut.
2. **Codex-style startup prompt**: when an update is available at launch, prompt every launch (no snooze/skip state). "Update" applies then CLOSES the app (user reopens; sidesteps Windows binary lock). Prompt shows "view changes" (release notes link) and "keep current version". Applies to TUI (new pre-Welcome screen) AND CLI (make the existing `[y/N]` the default, drop the `HGTRAN_AI_CONFIRM_UPDATE` env gate).
...
- `self-update`: CLI prompt becomes default (env gate removed); upgrade executor honors `HGTRAN_AI_CHANNEL`.
...
- **State store**: add fields to `InstallState` (`~/.hgtran-ai/state.json`): `last_update_check`, `pending_sync` (deferred-sync flag). No skip/snooze fields by decision.
...
- **Channel**: upgrade executor reads `HGTRAN_AI_CHANNEL` and routes beta to `@main`, stable to latest release.
...
| upgrade executor (`strategy.go` area) | Modified | Honor `HGTRAN_AI_CHANNEL` |
...
3. **Channel fix** — upgrade executor honors `HGTRAN_AI_CHANNEL`.
...
- One transition hgtran-ai release shipping this work (bootstrap, below).

## Bootstrap / Migration Plan

New behavior only exists in clients that already updated (chicken-and-egg). CLI users mostly auto-migrate — `selfUpdate` of the hgtran-ai binary works and is unaffected by the engram pin. TUI-only users see only a passive banner today and must act once manually. The version-independent escape hatch is the install script (`irm/curl … | iex/sh`), which always pulls latest regardless of installed logic. Plan: cut ONE transition hgtran-ai release with this work, advertise the one-time manual update out-of-band (Discord / README / release notes); from then on the new prompt + manifest carry the load. Transition cost is exactly one more hgtran-ai release; afterward engram releases reach users via runtime fetch with zero hgtran-ai release.

## Success Criteria

- [ ] An engram bump reaches users with ZERO hgtran-ai release (runtime fetch).
...
- [ ] `hgtran-ai upgrade` installs from the channel-correct source (beta → `@main`).
