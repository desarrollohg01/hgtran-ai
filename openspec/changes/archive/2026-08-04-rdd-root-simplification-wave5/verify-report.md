...
| Command | Exit | Result |
|---|---|---|
| `go build -trimpath -o <scratch>/hgtran-ai-c3 ./cmd/hgtran-ai` (at `63c0583a`) | 0 | `BUILD_OK` |
...
| `go test ./... -count=1` (bench module) | 0 | `ok github.com/desarrollohg01/hgtran-ai/bench 0.183s` |
...
`hgtran-ai review capture-result --cwd <repo> --lineage w8 --target <target> --lens <lens> --order <order>
--input <result.json>` (run with --preflight first to discover the exact subject hash to echo back), then
retry `hgtran-ai review finalize --cwd <repo> --lineage w8 --captured-results=true`
```
...
`.git/hgtran-ai/defect-reports/` **does not exist** in that repository at any point. In cycle 2 this same state wrote a report titled *"Hgtran AI reached a tool-internal fault state (`operation_outcome_unknown`) that should never happen"*.
...
`hgtran-ai review schema reviewer` requires both fields when `severity` is `BLOCKER`/`CRITICAL` (the `allOf`/`if`/`then` clause). The v3 capture path does not enforce that conditional, and the missing `causal_disposition` then routes to follow-up via W-9's rule. This one lives squarely in code fix cycle 3 touched (`newLineageCapturedFindings` and the v3 capture validator) and is closed by mirroring v2's existing refusal at capture time.
...
**Candidate**: chain tip `8e5f287a` on `feat/rdd-wave5-f2-v3-capture`, worktree `/home/hgtran/work/hgtran-ai-worktrees/rdd-wave5`
...
| Command | Exit | Result |
|---|---|---|
| `go build -trimpath -o <scratch>/hgtran-ai-c2 ./cmd/hgtran-ai` (at `8e5f287a`) | 0 | `BUILD_OK` |
...
| `go test ./... -count=1` (bench module) | 0 | `ok github.com/desarrollohg01/hgtran-ai/bench 0.180s` |
...
```
$ GENTLE_AI_RDD_NEW_LINEAGE=1 hgtran-ai review start --cwd <repo> --lineage med2
...
`hgtran-ai review capture-result --cwd <repo> --lineage hi1 --target <target> --lens review-risk --order 0 --preflight`

Error: capture binding does not match the frozen selected-lens order for lineage "hi1"; discover the exact
lens/order pairs with `hgtran-ai review capture-result … --preflight`
...
|---|---|---|---|
| receipt absent | `approved_without_receipt` | `hgtran-ai review finalize --lineage w7` | exit 0, `state: approved`, receipt re-issued; the gate then returns `allow` / `exact` |
| receipt present but tampered | `approved_receipt_corrupt` | `hgtran-ai review start --cwd <repo> --lineage <new-lineage-id>` | exit 0, `lineage_id: w7-successor`, `state: reviewing` |
...
```
driven via the real hgtran-ai binary: review start -> finalize -> validate --gate release
...
**Change**: `rdd-root-simplification-wave5` (Gate Cutover) | **Mode**: full spec-driven verification (proposal + design + tasks + 5 delta specs) | **Strict TDD**: active | **Store**: hybrid
**Candidate**: chain tip `1f875015` on `feat/rdd-wave5-p9-frozen-anchor-archive-gate`, worktree `/home/hgtran/work/hgtran-ai-worktrees/rdd-wave5`
...
| Command | Exit | Result |
|---|---|---|
| `go build -trimpath -o <scratch>/hgtran-ai-w5 ./cmd/hgtran-ai` (at `1f875015`) | 0 | `BUILD_OK`, `hgtran-ai dev` |
...
| `go test ./... -count=1` (bench module) | 0 | `ok github.com/desarrollohg01/hgtran-ai/bench 0.175s` |
...
```
$ GENTLE_AI_RDD_NEW_LINEAGE=1 hgtran-ai review start --cwd <repo> --lineage med1
...
$ GENTLE_AI_RDD_NEW_LINEAGE=1 hgtran-ai review finalize --cwd <repo> --lineage med1
...
- `review capture-result` binds the **v2 compact** store: preflighting the v3 lineage fails with `open <repo>/.git/hgtran-ai/review-transactions/v2/med1/review-state.json: no such file or directory`, while the lineage actually lives at `.git/hgtran-ai/review-transactions/v3/med1/`.
...
"result": "invalidated",
"reason": "facade review receipt is not available; run hgtran-ai review finalize --lineage med1 to produce one"
```
...
```
$ hgtran-ai sdd-attempt finish --cwd <repo> --change demo --expected-revision sha256:000... \
