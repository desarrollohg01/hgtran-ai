...
guarantees the upstream change MUST provide for hgtran-ai's probe-and-forward
...
once the flag ships, so `hgtran-ai`'s side-effect-free capability probe
(`ProbeProtocolFlag`) can detect support via `strings.Contains(stdout,
"--protocol")`.

- If the flag exists but is undocumented in `--help` output, hgtran-ai will
...
`hgtran-ai` invokes `engram setup --help` with `cmd.Stdin` unset (no TTY
...
- Research finding recorded in `design.md` Open Questions (live probe against
  ...
  stdin must fail fast or exit cleanly, not hang, so hgtran-ai's setup loop
...
`engram setup <slug>` runs at most once per unique slug (hgtran-ai dedups via
...
Antigravity both resolve to `gemini-cli`). When hgtran-ai forwards
...
- Upstream MUST treat `--protocol=slim` as scoped to the setup slug's hook
  ...
  granularity on the hgtran-ai side, by design.
- In hgtran-ai's current adapter matrix (design.md Decision 1), this is a
