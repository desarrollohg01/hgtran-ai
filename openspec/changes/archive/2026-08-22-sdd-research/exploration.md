...
12. Do not add a bench journey for prompt-only routing. If implementation changes native `sdd-status`/`sdd-continue` semantics, audit the journey corpus for stale old-route pins and prove the new journey through CI's locally built `hgtran-ai-bench run --binary ...` invocation; `go test ./bench` alone is declaration validation, not driven evidence.
