...
- [x] 1.3 Replace neutral expectations in `internal/components/persona/inject_test.go`: Claude writes Neutral style/settings, Kimi style is non-empty, OpenCode/Kilocode sync cleanup removes only `agent.hgtran`.
...
- [x] 3.3 Update OpenCode/Kilocode sync-managed neutral cleanup to remove only `agent.hgtran` while preserving sibling `agent` entries and tolerating malformed JSON.
