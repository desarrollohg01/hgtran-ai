...
3. **design.md doc hygiene**: The Decision-1/Decision-3 self-contradiction on residual persona filenames (Decision 1's table proposed new `*-hgtran-residual.md` files; Decision 3's tables and the actual File Changes table said "slim in place") was resolved correctly by implementation (3-source-corroborated reading), but design.md itself was not corrected post-hoc. Non-blocking, informational only.
