...
- **hgtran-pi's own adapter** — a separate repository. This wave ships the *provider* side (opaque transitions + capability declaration) plus *in-repo* consumers; Pi's thinning is hgtran-pi's chain, gated by its declared capability.
