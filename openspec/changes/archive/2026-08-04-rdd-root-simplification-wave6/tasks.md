...
- [x] F2.3 WARNING-4 (lost executable continuation): the digest-mismatch refusal in `repairAuthorityDispositionAtRepo` printed its cause (fix cycle 1) but, unlike base bb3c22a9, named no next action. Restored: "; run `hgtran-ai review repair --preflight` again for the current values".
