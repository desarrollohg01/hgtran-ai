...
// immediately runnable `hgtran-ai ...` invocation.
	...
	for _, test := range cases {
		for _, negotiated := range []bool{true, false} {
			...
			if negotiated {
				...
				usage := "Usage: hgtran-ai review " + operation + " [flags]"
			...
			if _, err := os.Stat(filepath.Join(run.sandbox.Repo, ".git", "hgtran-ai", "defect-reports")); !errors.Is(err, os.ErrNotExist) {
				if err == nil {
