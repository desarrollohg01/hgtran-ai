...
func TestInvokeTTYReportsPromptStartFailure(t *testing.T) {
	...
	sandbox.Binary = filepath.Join(t.TempDir(), "missing-hgtran-ai")
...
// so the review precondition can be tested without a real hgtran-ai.
func reviewModeStubBinary(t *testing.T, effective string) (string, string) {
	t.Helper()
	dir := t.TempDir()
	binary := filepath.Join(dir, "hgtran-ai-stub")
