...
// `hgtran-ai review start` remains compatibility-supported for
...
// sequence `hgtran-ai review start --base-ref <base-sha> --committed-only`
...
// route once, asserts the refusal names a runnable `hgtran-ai` continuation,
...
// name a runnable `hgtran-ai <verb> ...` continuation. It reuses the same
...
func compatAssertNamedRefusal(observation Observation, what string) error {
	...
	if !HasRunnableCommand(observation.Stdout + "\n" + observation.Stderr) {
		return fmt.Errorf("%s refused but named no runnable `hgtran-ai` continuation: stdout=%q stderr=%q",
...
// start` (with a space), not just to "hgtran-ai" generically -- a caller who
...
func compatAssertHyphenatedStartRefuses(_ *Sandbox, observation Observation) error {
	...
	if !strings.Contains(combined, "hgtran-ai review start") {
		return fmt.Errorf("review-start refusal did not name `hgtran-ai review start` (with a space): %q", combined)
	}
	return nil
}
...
func compatibilityJourneys() []Journey {
	journeys := []Journey{
		...
		},
		{
			...
			Review: reviewOptedIn,
			Title:  "The hyphenated `review-start` v1-compatibility verb refuses unconditionally and names `hgtran-ai review start`",
			Source: "internal/cli/review.go RunReviewStart: \"Read-only legacy v1 compatibility command. New authority is created with hgtran-ai review start.\"",
			Steps: []Step{
