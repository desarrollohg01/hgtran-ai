...
func TestPrintedConsecutiveRescopeRepairArgumentsPreservesLiteralBackticks(t *testing.T) {
	...
	stderr := "published consecutive-rescope record sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa is unreadable under normal replay; " + consecutiveRescopeRepairPrefix +
		"hgtran-ai sdd-attempt repair --cwd '" + workspace + "' --change '" + change + "'"
...
func TestRC1ConsecutiveRescopeProvenanceRefusesGeneratorCommandMutation(t *testing.T) {
	fixture := mutatedRC1ConsecutiveRescopeProvenance(t, func(manifest *rc1ConsecutiveRescopeManifest) {
		manifest.GeneratorCommands[1] = "go build -o hgtran-ai ./cmd/hgtran-ai"
	})
...
func TestRC1ConsecutiveRescopeGeneratorCommandsPinExactHistoricalWorkspace(t *testing.T) {
	...
	want := []string{
		...
		"go -C " + root + "/source build -trimpath -o " + root + "/hgtran-ai ./cmd/hgtran-ai",
		"mkdir " + root + "/repo",
		...
		"(cd " + root + " && ./hgtran-ai sdd-attempt begin --cwd " + root + "/repo --change consecutive-rescope-repair --expected-revision= --request-id begin-a --work-unit objective-a --evidence-goal prove-a --max-attempts 2 --max-changed-lines 20)",
		"(cd " + root + ` && ./hgtran-ai sdd-attempt finish --cwd ` + root + `/repo --change consecutive-rescope-repair --expected-revision "$(./hgtran-ai sdd-attempt status --cwd ` + root + `/repo --change consecutive-rescope-repair | jq -r '.revision')" --request-id finish-a-failed --outcome failed --evidence-revision sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --diagnosis "intentional failed zero-drift attempt" --harness-disposition reused --cleanup-evidence clean --process-evidence reproduced)`,
		"(cd " + root + ` && ./hgtran-ai sdd-attempt rescope --cwd ` + root + `/repo --change consecutive-rescope-repair --expected-revision "$(./hgtran-ai sdd-attempt status --cwd ` + root + `/repo --change consecutive-rescope-repair | jq -r '.revision')" --request-id rescope-a-b --work-unit objective-b --evidence-goal prove-b --max-attempts 1 --max-changed-lines 10 --reason "narrow A to B" --actor fixture-maintainer)`,
		"(cd " + root + ` && ./hgtran-ai sdd-attempt rescope --cwd ` + root + `/repo --change consecutive-rescope-repair --expected-revision "$(./hgtran-ai sdd-attempt status --cwd ` + root + `/repo --change consecutive-rescope-repair | jq -r '.revision')" --request-id rescope-b-c --work-unit objective-c --evidence-goal prove-c --max-attempts 1 --max-changed-lines 5 --reason "narrow B to C" --actor fixture-maintainer)`,
	}
