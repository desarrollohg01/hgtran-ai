...
func TestCompatAssertNamedRefusalFiresOnSuccessAndOnDeadEnd(t *testing.T) {
	if err := compatAssertNamedRefusal(Observation{ExitCode: 1, Stdout: "Error: cannot proceed; rerun with `hgtran-ai review start --contract x`"}, "test"); err != nil {
...
// compatAssertNamedRefusal: the refusal must specifically name `hgtran-ai
...
func TestCompatAssertHyphenatedStartRefusesRequiresTheSpacedVerb(t *testing.T) {
	ok := Observation{ExitCode: 1, Stderr: "legacy v1 review lineage is read-only: use hgtran-ai review start"}
	if err := compatAssertHyphenatedStartRefuses(nil, ok); err != nil {
		t.Fatalf("rejected a refusal that names hgtran-ai review start: %v", err)
	}
	wrongVerb := Observation{ExitCode: 1, Stderr: "refused; rerun `hgtran-ai review status --next-transition`"}
	if err := compatAssertHyphenatedStartRefuses(nil, wrongVerb); err == nil {
		t.Fatal("accepted a refusal that names a DIFFERENT hgtran-ai command, not `review start`; cw02 would pass without proving the redirect it claims")
	}
}
