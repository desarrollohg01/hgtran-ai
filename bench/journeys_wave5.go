...
// next step -- `hgtran-ai review start` -- even though the delivery used to
...
func requirePrePRMultiSegmentDenialNamesRunnableNextStep(_ *Sandbox, observation Observation) error {
	...
	if !strings.Contains(gate.Reason, "hgtran-ai review start") {
		return fmt.Errorf("multi-segment pre-PR denial reason = %q, want it to name the runnable next step (hgtran-ai review start)", gate.Reason)
	}
	return nil
}
