...
func issue1800State(r *journeyRun) ([]byte, issue1800Authority, error) {
	...
	stateBytes, err := os.ReadFile(filepath.Join(strings.TrimSpace(common), "hgtran-ai", "review-transactions", "v2", issue1800Lineage, "review-state.json"))
	var record struct {
...
func abandonIssue1800Authority(r *journeyRun) error {
	...
	if observation.ExitCode != 0 || json.Unmarshal([]byte(strings.TrimSpace(observation.Stdout)), &result) != nil ||
		...
		result.Record.QuarantinePath == "" || result.Record.Abandonment.Schema != "hgtran-ai.review-abandon-authorization/v2" ||
