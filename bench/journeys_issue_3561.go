...
func issue3561DanglingAncestorFixture(sandbox *Sandbox) error {
	...
	statePath := filepath.Join(sandbox.Home, ".hgtran-ai", "state.json")
...
func issue3561VerifyDoctor(sandbox *Sandbox, observation Observation) error {
	if observation.ExitCode != 0 {
		return fmt.Errorf("hgtran-ai doctor exited %d: %s", observation.ExitCode, firstLine(observation.Stderr))
	}
	if strings.Contains(observation.Stdout, "hgtran-ai sync") {
	...
	for _, want := range []string{sandbox.Scratch["issue-3561-config"], "dangling ancestor symlink " + ancestorPath, "inspect", "hgtran-ai doctor"} {
	...
	backupPath := filepath.Join(sandbox.Home, ".hgtran-ai", "backups")
