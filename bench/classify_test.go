...
func TestHasEnvelopeContinuation(t *testing.T) {
	cases := []struct {
		...
		{"execute operation with a runnable command", `{"next_transition":{"kind":"execute","execute":{"operation":"review.finalize","command":"hgtran-ai review finalize --lineage=review-1"}}}`, true},
		...
		{"execute command that is still a template", `{"next_transition":{"kind":"execute","execute":{"operation":"review.validate","command":"hgtran-ai review validate --gate <gate>"}}}`, false},
		{"execute command naming no arguments", `{"next_transition":{"kind":"execute","execute":{"operation":"review.status","command":"hgtran-ai"}}}`, false},
		{"execute with a command but no operation", `{"next_transition":{"kind":"execute","execute":{"operation":"","command":"hgtran-ai review status --cwd=."}}}`, false},
