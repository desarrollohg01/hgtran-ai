...
func captureProviderValidatorSlotWithResult(r *journeyRun, lineage string, passed bool) error {
	...
	start, err := json.Marshal(map[string]string{
		"schema": "hgtran-ai.provider-transport/v1", "operation": "start", "prompt": input.ProviderTask.Prompt,
	})
	if err != nil {
		return err
	}
	observation, err := r.runInteractive([]string{"review", "opencode-transport"}, true, func(reader *bufio.Reader, writer io.WriteCloser) error {
		...
		if err := json.Unmarshal([]byte(line), &prompt); err != nil || prompt.Schema != "hgtran-ai.provider-transport/v1" ||
		...
		completion, err := json.Marshal(map[string]string{
			"schema": "hgtran-ai.provider-transport/v1", "operation": "complete", "nonce": prompt.Nonce, "output": string(payload),
		})
		if err != nil {
			return err
		}
...
func capturedProviderSlotReported(stdout string, passed bool) bool {
	for _, line := range strings.Split(strings.TrimSpace(stdout), "\n") {
		...
		if json.Unmarshal([]byte(frame.Output), &closure) == nil &&
			closure.Schema == "hgtran-ai.review-last-event-closure/v1" && closure.Operation == "review/capture-validation" &&
