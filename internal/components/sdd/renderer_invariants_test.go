...
var currentOpenCodeOrchestratorSections = []orchestratorContractSection{
	...
	},
	{
		...
		marker: "#### Hgtran AI Provider Defect Handoff (MANDATORY)",
		sentinels: []string{
	...
	},
	{
		...
		sentinels: []string{
			"hgtran-ai.sdd-integration.consent/v1",
...
func TestOpenCodeBaseOrchestratorPreservesCurrentContract(t *testing.T) {
	...
	visibilityMarker := "<!-- hgtran-ai:" + openCodeDelegationVisibilitySectionID + " -->"
...
func TestOpenCodeBaseInjectionBindsAssignmentsAndPreservesContract(t *testing.T) {
	...
	assignments := map[string]model.ModelAssignment{
		"hgtran-orchestrator": {ProviderID: "openai", ModelID: "gpt-5.1", Effort: "high"},
	...
	prompt := agentPrompt(t, agentsMap, "hgtran-orchestrator")
	...
	for agentName, wantModel := range map[string]string{
		"hgtran-orchestrator": "openai/gpt-5.1",
	...
	if variant := agentsMap["hgtran-orchestrator"].(map[string]any)["variant"]; variant != "high" {
		t.Fatalf("hgtran-orchestrator variant = %#v, want high", variant)
	}
}
