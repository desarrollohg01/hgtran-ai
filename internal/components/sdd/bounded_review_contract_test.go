...
func boundedReviewRequiredClausesFor(agent model.AgentID) []string {
	return append(captureTransportClausesFor(agent), []string{
		...
		"hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent " + string(agent) + " --next-transition",
		...
		"typed `hgtran-ai.review-integration.consent/v3` envelope",
...
func TestBoundedReviewStopInventoryIsCompleteWithoutRepeatingStatus(t *testing.T) {
	...
	for _, want := range []string{
		"`D` means `hgtran-ai review mode disable --scope clone --cwd <B>`",
	...
	if strings.Contains(inventory, "hgtran-ai review status --cwd") {
		t.Fatal("stop inventory repeats the canonical STATUS command instead of using S")
	}
	canonicalStatus := "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent " + runtimeAgentIDPlaceholder + " --next-transition"
	...
	for name, required := range controllers {
		...
		t.Run(name, func(t *testing.T) {
			...
			if name == "orchestrator" {
				clauses = append(clauses, "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent "+string(model.AgentOpenCode)+" --next-transition")
			} else if strings.Contains(content, "hgtran-ai review status --cwd <repo>") {
...
func TestPreservedSharedOrchestratorSubstitutesRuntimeAgentIdentity(t *testing.T) {
	...
	if !strings.Contains(rendered, "Bind this to the dedicated `hgtran-orchestrator` agent only.") {
...
func TestPreservedSharedOrchestratorLeavesRetiredWorkCommandsUntouched(t *testing.T) {
	...
	retired := []string{
		"hgtran-ai work-capabilities --cwd <repo> --contract hgtran-ai.work-capabilities/v2 --json",
		"hgtran-ai work-start --cwd <repo> --contract hgtran-ai.work-start/v1 --json",
	}
	rendered := renderPreservedOpenCodeOrchestratorPrompt(
		...
		for _, lens := range []string{"risk", "readability", "reliability", "resilience"} {
			...
			t.Run(family+"/"+lens, func(t *testing.T) {
				...
				if family == "claude" {
					...
					for _, forbidden := range []string{"OpenCode tasks begin", "hgtran-ai review inspect-candidate"} {
						if strings.Contains(content, forbidden) {
...
func TestOpenCodeAndClaudeApplyCommandsUseTheAtomicLifecycle(t *testing.T) {
	for _, path := range []string{"opencode/commands/sdd-apply.md", "claude/commands/hgtran-sdd-apply.md"} {
		t.Run(path, func(t *testing.T) {
			...
			if strings.Contains(content, "hgtran-ai review status --cwd <repo>") {
				t.Fatalf("%s duplicates the canonical STATUS command", path)
			}
			for _, forbidden := range []string{"runs `hgtran-ai review start --cwd <repo>`", "Reuse a valid receipt", "reviewGate.result: allow"} {
				if strings.Contains(content, forbidden) {
...
func TestOpenCodeAndClaudeArchiveInstructionsDoNotGateOnReviewAuthority(t *testing.T) {
	for _, path := range []string{"opencode/commands/sdd-archive.md", "claude/commands/hgtran-sdd-archive.md"} {
		t.Run(path, func(t *testing.T) {
