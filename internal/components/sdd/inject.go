...
type InjectOptions struct {
	...
	// opencode.json agent.hgtran-orchestrator.prompt value during sync.
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// orchestrator instructions must be scoped to the OpenCode hgtran-orchestrator agent only,
	...
	// 2b. OpenCode /sdd-* commands reference agent: hgtran-orchestrator.
	...
	if adapter.Agent() == model.AgentOpenCode {
		...
		if !hasOpenCodeAgentKey(settingsText, "hgtran-orchestrator") {
			// In-memory check failed — try reading from disk as last resort.
			if diskBytes, readErr := os.ReadFile(settingsPath); readErr == nil {
				settingsText = string(diskBytes)
			}
			if !hasOpenCodeAgentKey(settingsText, "hgtran-orchestrator") {
				return InjectionResult{}, fmt.Errorf("post-check: %q missing hgtran-orchestrator agent definition — OpenCode /sdd-* commands will fail", settingsPath)
			}
		}
...
func inlineOpenCodeSDDPrompts(overlayBytes []byte, homeDir, settingsPath string, agent model.AgentID, preserveExistingOrchestratorPrompt bool, renderOptions OrchestratorRenderOptions, codeGraphGuidance string) ([]byte, error) {
	...
	orchestratorRaw, ok := agentsMap["hgtran-orchestrator"]
	if !ok {
		return overlayBytes, nil
	}
	...
	if preserveExistingOrchestratorPrompt {
		existingPrompt, err := readOpenCodeAgentPrompt(settingsPath, "hgtran-orchestrator")
		if err != nil {
			return nil, err
		}
		if existingPrompt == "" {
...
func restoreKilocodeManagedAgentTools(agentsMap map[string]any) {
	for name, raw := range agentsMap {
		...
		switch {
		case name == "hgtran-orchestrator" || strings.HasPrefix(name, "sdd-orchestrator-"):
...
func ensurePreservedOpenCodeResearchLifecycle(prompt string) string {
	if strings.Contains(prompt, "<!-- hgtran-ai:sdd-research-lifecycle -->") && strings.Contains(prompt, researchLifecycleContract()) {
		return prompt
	}
...
// stripOpenCodeBackgroundPolicy removes only the complete Hgtran AI-owned block.
...
const nativeReviewAuthorityRule = "7. **Authority rule**: use selectorless `hgtran-ai review status` only to preflight the current worktree" +
...
func stripOpenCodeNativeFallbackAgents(overlayBytes []byte) ([]byte, error) {
	...
	if orchestrator, ok := agents["hgtran-orchestrator"].(map[string]any); ok {
...
func removeManagedOpenCodeAgentTools(baseJSON, overlay []byte) ([]byte, error) {
	keys := append([]string{"hgtran-orchestrator", "general", "explore"}, ProfilePhaseOrder()...)
...
// hgtran-orchestrator primary agent; named profile agents such as
// sdd-orchestrator-cheap intentionally remain untouched because they are
// generated profile-specific coordinators. The old OpenCode "hgtran" agent
// key is revoked and is removed during sync; if it clearly contains the old SDD
// conductor prompt and no hgtran-orchestrator exists yet, its prompt is migrated
...
func migrateLegacyOpenCodeSDDOrchestrator(baseJSON []byte) ([]byte, error) {
	...
	revokedGentleman, hasRevokedGentleman := agentsMap["hgtran"]
	...
	if _, hasGentleOrchestrator := agentsMap["hgtran-orchestrator"]; !hasGentleOrchestrator && hasLegacy {
		agentsMap["hgtran-orchestrator"] = legacy
	}
	delete(agentsMap, "sdd-orchestrator")
	if hasRevokedGentleman {
		delete(agentsMap, "hgtran")
	}
...
// current base coordinator key: hgtran-orchestrator. Named profile keys remain unchanged.
func normalizeOpenCodeSDDModelAssignments(assignments map[string]model.ModelAssignment) map[string]model.ModelAssignment {
	...
	if _, hasGentleOrchestrator := assignments["hgtran-orchestrator"]; hasGentleOrchestrator {
		return assignments
	}
	...
	normalized["hgtran-orchestrator"] = legacyAssignment
	return normalized
}
