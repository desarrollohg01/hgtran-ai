...
func TestOpenCodeResearchCommandHasExplicitTaskPermissionAndDefaultDenial(t *testing.T) {
	...
	for _, required := range []string{"agent: hgtran-orchestrator", "hidden `sdd-research` sub-agent", "SDD Session Preflight must already be complete"} {
	...
	for _, path := range []string{"opencode/sdd-overlay-single.json", "opencode/sdd-overlay-multi.json"} {
		t.Run(path, func(t *testing.T) {
			...
			orchestrator := agents["hgtran-orchestrator"].(map[string]any)
...
func TestSlashCommandPathsNamespaceClaudeOnly(t *testing.T) {
	...
	if claude[0] != filepath.Join(dir, "hgtran-sdd-init.md") || claude[1] != filepath.Join(dir, "sdd-init.md") {
	...
	if got := LegacyClaudeCommandPath(model.AgentOpenCode, "cmds", "hgtran-sdd-init.md"); got != "" {
