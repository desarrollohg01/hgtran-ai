...
func TestGeneratedOrchestratorInstructionsNameTheExecutingRuntime(t *testing.T) {
	...
	for _, agent := range catalog.AllAgents() {
		t.Run(string(agent.ID), func(t *testing.T) {
			...
			if strings.Contains(content, "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2") {
...
func TestAdvertisedRenderedReviewProtocolsBindRuntimeOnce(t *testing.T) {
	for _, agent := range catalog.AllAgents() {
		...
		t.Run(string(agent.ID), func(t *testing.T) {
			...
			status := "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent " + string(agent.ID) + " --next-transition"
