...
func TestStdioHandshake_MissingBinary(t *testing.T) {
	if err := stdioHandshake(context.Background(), stdioProbeTimeout, "hgtran-ai-test-no-such-binary"); err == nil {
...
func TestProbeStdio_UsesExactPersistedCommand(t *testing.T) {
	...
	if err := ProbeStdio(context.Background(), 0, "/opt/hgtran-engram/bin/engram", "mcp", "--tools=custom"); err != nil {
		t.Fatalf("ProbeStdio() error = %v", err)
	}
	if gotName != "/opt/hgtran-engram/bin/engram" {
