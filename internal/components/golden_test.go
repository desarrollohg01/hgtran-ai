...
import (
	...
	"bitbucket.org/hgt_development/hgtran-ai/v2/internal/agents/kiro"
	...
	"bitbucket.org/hgt_development/hgtran-ai/v2/internal/agents/windsurf"
...
func TestGoldenConfigs(t *testing.T) {
	...
	presets := []presetMapping{
		{Preset: "full-hgtran", Skills: toStringSlice(skills.SkillsForPreset("full-hgtran"))},
...
func TestGoldenSDD_Claude(t *testing.T) {
	...
	for _, name := range []string{
		"hgtran-sdd-apply", "hgtran-sdd-archive", "hgtran-sdd-continue", "hgtran-sdd-explore",
		"hgtran-sdd-ff", "hgtran-sdd-init", "hgtran-sdd-new", "hgtran-sdd-onboard", "hgtran-sdd-research", "hgtran-sdd-status", "hgtran-sdd-verify",
	} {
...
func TestGoldenSDD_OpenCode_Multi(t *testing.T) {
	...
	if strings.Contains(string(settingsJSON), "<!-- hgtran-ai:opencode-background-subagents -->") {
...
func TestGoldenPersona_Claude_Gentleman(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("persona.Inject(claude, hgtran) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(claude, hgtran) changed = false")
	}

	claudeMD := readTestFile(t, filepath.Join(home, ".claude", "CLAUDE.md"))
	assertGolden(t, "persona-claude-hgtran.golden", claudeMD)

	outputStyle := readTestFile(t, filepath.Join(home, ".claude", "output-styles", "hgtran.md"))
	assertGolden(t, "persona-claude-hgtran-outputstyle.golden", outputStyle)

	settingsJSON := readTestFile(t, filepath.Join(home, ".claude", "settings.json"))
	assertGolden(t, "persona-claude-hgtran-settings.golden", settingsJSON)
}
...
func TestGoldenPersona_OpenCode_Gentleman(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("persona.Inject(opencode, hgtran) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(opencode, hgtran) changed = false")
	}

	agentsMD := readTestFile(t, filepath.Join(home, ".config", "opencode", "AGENTS.md"))
	assertGolden(t, "persona-opencode-hgtran.golden", agentsMD)
}
...
func TestGoldenPersona_Windsurf_Gentleman(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("persona.Inject(windsurf, hgtran) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(windsurf, hgtran) changed = false")
	}

	globalRules := readTestFile(t, filepath.Join(home, ".codeium", "windsurf", "memories", "global_rules.md"))
	assertGolden(t, "persona-windsurf-hgtran.golden", globalRules)
}
...
func TestGoldenPersona_Kiro_Gentleman(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("persona.Inject(kiro, hgtran) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(kiro, hgtran) changed = false")
	}

	promptPath := adapter.SystemPromptFile(home)
	instructionsFile := readTestFile(t, promptPath)
	assertGolden(t, "persona-kiro-hgtran.golden", instructionsFile)
}
...
func TestGoldenPersona_Antigravity_Gentleman(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("persona.Inject(antigravity, hgtran) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(antigravity, hgtran) changed = false")
	}

	rulesFile := readTestFile(t, filepath.Join(home, ".gemini", "GEMINI.md"))
	assertGolden(t, "persona-antigravity-hgtran.golden", rulesFile)
}
