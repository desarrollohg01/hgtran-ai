...
func failureManualActions(failures []operationFailure, batch []model.AgentID, homeDir string) []string {
	...
	for _, failure := range failures {
		...
		command := "hgtran-ai uninstall --all --yes"
		if len(retry) > 0 {
			command = "hgtran-ai uninstall " + strings.Join(retry, " ") + " --yes"
		}
...
func (s *Service) componentOperations(adapter agents.Adapter, componentID model.ComponentID) ([]operation, []string, error) {
	...
	case model.ComponentPersona:
		...
		if adapter.SupportsOutputStyles() {
			path := filepath.Join(adapter.OutputStyleDir(homeDir), "hgtran.md")
			targets = append(targets, path)
		...
		if path := adapter.SettingsPath(homeDir); path != "" {
			...
			if adapter.Agent() == model.AgentOpenCode {
				jsonPaths = append(jsonPaths, jsonPath{"agent", "hgtran"})
			}
	...
	case model.ComponentOpenCodeGentleLogo:
		pluginPath := filepath.Join(homeDir, ".config", "opencode", "tui-plugins", "hgtran-logo.tsx")
