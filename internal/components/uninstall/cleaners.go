...
func removeManagedPersonaPreamble(content string) (string, bool) {
	...
	markerIdx := strings.Index(normalized, "<!-- hgtran-ai:")
...
func looksLikeManagedPersonaPrefix(prefix string) bool {
	if strings.Contains(prefix, "name: Hgtran AI Persona") && strings.Contains(prefix, "description: Teaching-oriented persona") {
		return true
	}
