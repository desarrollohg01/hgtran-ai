	...
	if before.CLI == AvailabilityAvailable && len(targets) > 0 {
		if installed, ok := codeGraphInstalledVersion(before.CLIPath); ok && codeGraphVersionLess(installed, codeGraphUpstreamVersion) {
			...
			result.ManualActions = append(result.ManualActions, fmt.Sprintf(
				"CodeGraph %s is older than the %s target contract Hgtran AI is written against, so agent targets were left to CodeGraph's own detection. Run `npm install -g @colbymchenry/codegraph@latest` (or `pnpm add -g @colbymchenry/codegraph@latest`) and rerun Hgtran AI to get explicit target selection.",
