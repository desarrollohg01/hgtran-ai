...
func TestInjectMergesThemeOverlayIntoAdapterSettings(t *testing.T) {
	...
	if root.Theme != "hgtran" {
		t.Fatalf("theme = %q, want hgtran", root.Theme)
	}
	...
	if _, err := os.Stat(filepath.Join(home, ".claude", "themes", "hgtran.json")); !os.IsNotExist(err) {
...
func TestInjectVisualThemesIsIdempotentForClaude(t *testing.T) {
	...
	path := filepath.Join(home, ".claude", "themes", "hgtran.json")
...
func TestInjectVisualThemesPreservesGentlemanClaudeTheme(t *testing.T) {
	...
	themePath := filepath.Join(home, ".claude", "themes", "hgtran.json")
	...
	if root.Name != "hgtran" || root.Base != "dark" {
		t.Fatalf("theme identity = %q/%q, want hgtran/dark", root.Name, root.Base)
	}
...
const (
	gentlemanClaudeFixture   = `{"name":"hgtran","base":"dark","overrides":{"diffAdded":"#3F4A2D","diffRemoved":"#5C3838","diffAddedWord":"#76946A","diffRemovedWord":"#C34043","chromeYellow":"#DCA561","briefLabelYou":"#DCA561","rainbow_yellow":"#DCA561","yellow_FOR_SUBAGENTS_ONLY":"#DCA561"}}`
...
func TestInjectVisualThemesWritesExpectedAssets(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{filepath.Join(home, ".claude", "themes", "hgtran.json"), gentlemanClaudeFixture, &claudeTheme{}},
		{filepath.Join(home, ".claude", "themes", "hgtran-cute.json"), cuteClaudeFixture, &claudeTheme{}},
		{filepath.Join(xdg, "opencode", "themes", "hgtran.json"), gentlemanOpenCodeFixture, &openCodeTheme{}},
		{filepath.Join(xdg, "opencode", "themes", "hgtran-cute.json"), cuteOpenCodeFixture, &openCodeTheme{}},
	} {
