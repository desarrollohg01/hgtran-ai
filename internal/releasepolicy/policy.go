...
func validateArtifacts(root string, payload []byte, markerTime time.Time, contractSemver string) error {
	...
	for _, item := range byType["Archive"] {
		if item.Name == "hgtran-ai-release-provenance-v1.tar.gz" {
		...
		if strings.HasPrefix(item.Name, "hgtran-ai-review-provider-contract-") {
			artifactSemver := strings.TrimSuffix(strings.TrimPrefix(item.Name, "hgtran-ai-review-provider-contract-"), ".tar.gz")
...
archives:
  ...
  - id: review-provider-contract
    ...
    formats:
      - tar.gz
    name_template: "hgtran-ai-review-provider-contract-{{ .Env.PROVIDER_CONTRACT_SEMVER }}"
    files:
  ...
  - id: release-provenance
    ...
    formats:
      - tar.gz
    name_template: "hgtran-ai-release-provenance-v1"
    files:
  ...
  preflight:
    ...
    steps:
      ...
      - name: Verify provider contract bundle
        run: go run ./internal/providercontractbundlecmd verify --archive "dist/hgtran-ai-review-provider-contract-${PROVIDER_CONTRACT_SEMVER}.tar.gz"
