...
// Example: <!-- hgtran-ai:custom-agent:my-skill -->
const markerFormat = "<!-- hgtran-ai:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /hgtran-ai:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /hgtran-ai:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- hgtran-ai:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- hgtran-ai:custom-agent:"
	suffix := " -->"
