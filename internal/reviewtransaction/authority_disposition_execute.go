...
func readBackAuthorityDisposition(ctx context.Context, root string, record CompactReclaimRecord) (CompactReclaimRecord, error) {
	if record.Status != CompactReclaimCommitted {
		return record, fmt.Errorf("authority disposition execution refused: readback observed a non-committed record; run `hgtran-ai review inspect-authority --cwd %s` and escalate the report", pathquote.Quote(root))
	}
	...
	if !report.Complete {
		return record, fmt.Errorf("authority disposition execution refused: retained-graph readback is incomplete; run `hgtran-ai review inspect-authority --cwd %s` and escalate the report", pathquote.Quote(root))
	}
	closureMembers := authorityDispositionClosureMembers(record)
	for _, edge := range report.Edges {
		if member := edge.PredecessorLineageID; closureMembers[member] {
			return record, fmt.Errorf("authority disposition execution refused: retained graph still references quarantined closure member %q; run `hgtran-ai review inspect-authority --cwd %s` and escalate the report", member, pathquote.Quote(root))
		}
