...
// preRelocationCloneMode reports the clone-local mode exactly as a hgtran-ai
...
// only this build can see reports success while every older hgtran-ai on the
...
func TestCloneScopeDisableAppliesToPreRelocationBuilds(t *testing.T) {
	...
	if status := preRelocationCloneMode(t, ctx, repo); status.Enabled() {
		t.Fatalf("a pre-relocation hgtran-ai still reports reviews on: %#v; the clone-scope disable reported success without applying", status)
	}
...
func TestCloneScopeEnableAlsoReachesPreRelocationBuilds(t *testing.T) {
	...
	if status := preRelocationCloneMode(t, ctx, repo); !status.Enabled() {
		t.Fatalf("a pre-relocation hgtran-ai still reports reviews off: %#v; the clone-scope enable never reached it", status)
	}
	assertCloneModeLocationsAgree(t, ctx, repo)
}
...
func TestCloneScopeRerunRepairsABuildLocalDisable(t *testing.T) {
	...
	if status := preRelocationCloneMode(t, ctx, repo); status.Enabled() {
		t.Fatalf("rerunning the disable left a pre-relocation hgtran-ai enforcing review: %#v", status)
	}
	assertCloneModeLocationsAgree(t, ctx, repo)
...
// arithmetic across two locations. A hgtran-ai from before the relocation
...
func TestCloneScopeWriteSupersedesHigherPreRelocationGenerations(t *testing.T) {
	...
	if status := preRelocationCloneMode(t, ctx, repo); !status.Enabled() {
		t.Fatalf("a pre-relocation hgtran-ai kept its own older record: %#v", status)
	}
	assertCloneModeLocationsAgree(t, ctx, repo)
}
...
// pre-relocation hgtran-ai can read it, and the publish there failed anyway.
...
func TestCloneScopeWriteRefusesToClaimSuccessOnAPartialPublish(t *testing.T) {
	...
	if reported.Mode != RDDModeUnset || !strings.Contains(err.Error(), "hgtran-ai review mode enable --scope clone") {
...
// pre-relocation hgtran-ai actually reads. That build keeps enforcing the
...
func TestCloneScopeWriteNeverClaimsTheMachineOverAnUnscannableMirror(t *testing.T) {
	...
	// A pre-relocation hgtran-ai then published generations of its own, so the
	...
	if status.Reach == RDDModeReachMachine {
		if pre := preRelocationCloneMode(t, ctx, repo); !pre.Enabled() {
			t.Fatalf("the write claimed machine-wide reach while a pre-relocation hgtran-ai still reads reviews off: %#v", pre)
		}
	}
...
func TestCloneScopeConcurrentWritesLeaveBothLocationsAgreeing(t *testing.T) {
	...
	if status := preRelocationCloneMode(t, ctx, repo); status.Enabled() {
		t.Fatalf("concurrent writers left a pre-relocation hgtran-ai enforcing review: %#v", status)
	}
	assertCloneModeLocationsAgree(t, ctx, repo)
}

// assertCloneModeLocationsAgree checks the property the two locations exist to
// hold jointly: the same head record, byte for byte. A pre-relocation hgtran-ai
...
func damageCloneModeLegacyTreeForTest(t *testing.T, repo string) {
	t.Helper()
	hgtran := filepath.Join(repo, ".git", "hgtran-ai")
	if err := os.MkdirAll(hgtran, 0o700); err != nil {
		t.Fatal(err)
	}
	authority := filepath.Join(hgtran, rddModeLegacySwitchDirectory)
