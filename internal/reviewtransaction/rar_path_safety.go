...
func ensureRARRepositoryRoot(commonDir, root string, create bool) error {
	...
	// rar-authority and every descendant are owner-only; hgtran-ai and
...
// review-transactions under hgtran-ai. It reuses this file's walk, permission
...
func ensureRARSwitchRoot(commonDir, root string, create bool) error {
	...
	want := filepath.Join("hgtran-ai", rddModeSwitchDirectory, rarAuthorityDirectory, rarAuthorityVersion)
	return ensureRARDirectoryChain(commonDir, root, want, 2, create)
}
