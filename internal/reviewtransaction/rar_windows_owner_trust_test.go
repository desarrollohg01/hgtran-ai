...
// ownerOnlyDirectory is exactly what hgtran-ai writes for an owner-only RAR
...
func TestRARWindowsOwnerOnlyMismatch(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "the descriptor hgtran-ai writes for a directory",
			observed:  ownerOnlyDirectory(),
		...
		{
			name: "the descriptor hgtran-ai writes for a file",
			observed: withOwnerOnlyDirectory(func(observed *rarWindowsOwnerOnlyDescriptor) {
