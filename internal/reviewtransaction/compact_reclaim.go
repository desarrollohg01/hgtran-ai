...
func compactReclaimAuthorityRefusal(ctx context.Context, repo, dir, lineageID, artifact string) error {
	...
	if loadErr != nil {
		if os.IsNotExist(loadErr) {
			return fmt.Errorf("%s The entry holds no readable review-state.json beside that artifact, so nothing can prove the artifact never carried authority, and no advertised operation admits this shape today."+
				" Capture the complete machine-readable diagnosis with `hgtran-ai review inspect-authority --cwd %s` and escalate that report", refused, pathquote.Quote(repo))
		}
		return fmt.Errorf("%s Its record cannot be loaded (%v) — inspection classifies it %s — and no advertised operation admits an unreadable record:"+
			...
			" Capture the complete machine-readable diagnosis with `hgtran-ai review inspect-authority --cwd %s` and escalate that report",
	...
	if eligibilityErr == nil && eligibility.Eligible {
		return fmt.Errorf("%s The entry is eligible for abandonment, so `hgtran-ai review abandon` quarantines it whole: %s",
...
const AuthorityDispositionClosureManifestSchema = "hgtran-ai.review-authority-disposition-closure-manifest/v1"
