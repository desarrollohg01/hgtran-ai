...
//
// Review authority accumulates without bound under <git-common-dir>/hgtran-ai:
...
// tree, still written for hgtran-ai builds installed before #2882 that read
...
// state hgtran-ai itself writes. A category outside that coverage cannot be
...
// projection.
const StoreResetReportSchema = "hgtran-ai.review-store-reset/v1"
...
// storeResetTarget names one path under <git-common-dir>/hgtran-ai and why it
...
var storeResetRemovableTargets = []storeResetTarget{
	...
	},
	{
		...
		reason: "review graph store written by the hgtran-pi adapter, outside this command's lease and in-flight coverage; removed only because --include-adapter-reviews was given",
		withheldReason: "review graph store the hgtran-pi adapter writes and this command cannot vouch for: " +
...
// hgtran-ai code has ever written -- which is itself the reason.
var storeResetPreservedTargets = []storeResetTarget{
	...
	},
	{
		...
		reason: "no hgtran-ai code writes or reads this path, so its contents were placed by hand; a reset never removes what the product did not create",
	},
	{
...
func (err *StoreResetInFlightError) Error() string {
	...
	return fmt.Sprintf(
		"review store reset refused: %d review(s) have not reached a terminal state: %s; finish or abandon them, or run `hgtran-ai review store-reset --cwd %s --confirm --include-in-flight` to remove them anyway",
...
// storeResetRoot resolves <git-common-dir>/hgtran-ai for one clone.
//
...
func storeResetRoot(ctx context.Context, repo string) (root, repository string, err error) {
	...
	return filepath.Join(identity.GitCommonDir, "hgtran-ai"), identity.RepositoryRoot, nil
}
