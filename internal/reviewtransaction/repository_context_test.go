...
func TestHistoricalRctx1ContextIsReadOnly(t *testing.T) {
	...
	before, err := os.ReadFile(filepath.Join(os.Getenv("HOME"), ".hgtran-ai", "review-contexts", "v1", handle+".json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	after, err := os.ReadFile(filepath.Join(os.Getenv("HOME"), ".hgtran-ai", "review-contexts", "v1", handle+".json"))
...
func TestRctx2HandleResolvesAgainstTheCallerRepositoryWithoutMutation(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(os.Getenv("HOME"), ".hgtran-ai", "review-contexts")); !errors.Is(err, os.ErrNotExist) {
