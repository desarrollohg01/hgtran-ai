...
func (builder SnapshotBuilder) buildHeadWithIntended(ctx context.Context, intended []string) (string, string, error) {
	...
	temp, err := os.CreateTemp(gitDir, ".hgtran-ai-review-index-*")
	if err != nil {
		return "", "", err
	}
	tempIndex := temp.Name()
...
const intendedUntrackedInventoryCommand = "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent <runtime> --next-transition"
...
func intendedUntrackedInventoryDigest(paths []string) string {
	hash := sha256.New()
	writeLengthPrefixed(hash, []byte("hgtran-ai.intended-untracked-inventory/v1"))
	for _, path := range paths {
...
func (builder *SnapshotBuilder) buildCurrentChanges(ctx context.Context, intended []string, allowStagedIntended bool, projection Projection) (string, string, string, error) {
	...
	// Keep the private index beside Git's writable control files. A restricted
	// integration environment may not provide an accessible process temp dir.
	temp, err := os.CreateTemp(filepath.Dir(indexPath), ".hgtran-ai-review-index-*")
	if err != nil {
		return "", "", "", err
	}
	tempIndex := temp.Name()
...
func snapshotIdentityForProjection(kind TargetKind, projection Projection, baseTree, candidateTree, pathsDigest, proof string, intended, ledgerIDs []string) string {
	...
	if kind == TargetBaseWorkspaceOverlay {
		hash.Write([]byte("hgtran-ai.review-snapshot/base-workspace-overlay/v2\x00"))
	} else if projection == ProjectionStaged {
		hash.Write([]byte("hgtran-ai.review-snapshot/v4\x00"))
	} else {
		hash.Write([]byte("hgtran-ai.review-snapshot/v3\x00"))
	}
...
// refusal:by-design world-action: unexpected Git diagnostics require repairing the repository or its environment; no Hgtran AI command can safely infer that repair.
