...
func repairAuthorityDispositionAtRepo(ctx context.Context, repo, planDigest, inventoryRevision, actor, reason, authorization string, selector *AuthorityDispositionSelector) (CompactReclaimRecord, error) {
	...
	if plan.PlanDigest != planDigest || plan.AuthorityInventoryRevision != inventoryRevision {
		...
		// ("run `hgtran-ai review repair --preflight` again for the current
		...
		return CompactReclaimRecord{}, fmt.Errorf("%w: submitted plan_digest/inventory_revision does not match the current provider-derived plan; run `hgtran-ai review repair --preflight` again for the current values", ErrConcurrentUpdate)
	}
