...
func deriveCompactCapturePhaseRevision(state CompactState) (string, error) {
	...
	sum := sha256.Sum256(append([]byte("hgtran-ai.review-capture-phase/v1\x00"), payload...))
...
var ErrInvalidFindingLocation = errors.New("invalid reviewer finding location; correct it to repository/path:<positive-line> or repository/path:<positive-start>-<positive-end> before running hgtran-ai review capture-result again")
