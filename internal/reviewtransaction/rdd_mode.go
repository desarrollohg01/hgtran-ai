...
var (
	...
	// hgtran-ai reads. It is the opposite of a fallback: it exists so a
	// half-applied kill switch can never be reported as a working one.
	ErrRDDModePartiallyApplied = errors.New("clone-local review mode was not applied for every hgtran-ai on this machine")
...
// every hgtran-ai installed before that relocation kept reading its own
...
const (
	...
	// RDDModeReachMachine means every hgtran-ai installed on this machine
	...
	// reached. A hgtran-ai installed before the relocation keeps reading the
...
// reviewModeEnableForSource names the exact `hgtran-ai review mode enable`
...
func reviewModeEnableForSource(source RDDModeSource) string {
	const enable = "hgtran-ai review mode enable --scope="
...
// hgtran-ai installations on this machine now disagree about whether reviews
...
func (err *RDDModePartialApplyError) Error() string {
	...
	return fmt.Sprintf(
		"%v: this clone %s receipt-driven development for this hgtran-ai, but publishing the same decision under hgtran-ai/%s/%s/%s/%s failed, so a hgtran-ai installed before the switch moved still reads the value it already has there and keeps enforcing it: %v; rerun `hgtran-ai review mode %s --scope clone` to publish it in both places",
...
func SetCloneLocalRDDMode(
	...
	// new generation of its own -- but it is not finished until every hgtran-ai
	...
	// because that is where the other hgtran-ai installations on this machine
	...
	// version of hgtran-ai without a lock-order cycle.
	...
	// of them has published further. A pre-relocation hgtran-ai that wrote its
	...
	// The same exact bytes at the same slot: a pre-relocation hgtran-ai parses,
...
// hgtran-ai fails closed on, so refusing there would only re-close the exit
...
func openCloneLocalRDDModeMirror(ctx context.Context, repo string) *cloneLocalRDDModeMirror {
	...
	// hgtran-ai cannot read either, and refusing over it would re-close the
	...
	if headErr != nil {
		...
		// into it: below the record a pre-relocation hgtran-ai actually reads,
...
// decides reports whether a pre-relocation hgtran-ai reading this location
...
// and therefore where every hgtran-ai released before that move still reads it.
...
func cloneLocalRDDModeRoot(ctx context.Context, repo string, create bool) (string, error) {
	...
	base := filepath.Join(identity.GitCommonDir, "hgtran-ai", rddModeSwitchDirectory, rarAuthorityDirectory, rarAuthorityVersion)
...
// every hgtran-ai installed before the relocation still looks, so a clone-scope
