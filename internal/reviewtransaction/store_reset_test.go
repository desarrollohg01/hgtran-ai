...
// storeResetGentleAIRoot returns <git-common-dir>/hgtran-ai for a fixture repo.
...
func seedStoreResetLineageAt(t *testing.T, root, lineage string, state State) {
	...
	writeStoreResetFile(t, filepath.Join(root, "review-transactions", "v2", lineage, "review-state.json"),
		`{"schema":"hgtran-ai.review-state-record/v2","revision":"sha256:00","state":{"schema":"hgtran-ai.review-state/v2","lineage_id":"`+lineage+`","state":"`+string(state)+`"}}`+"\n")
}
...
func seedStoreResetKillSwitch(t *testing.T, repo string) (current, legacy string) {
	...
	writeStoreResetFile(t, current, `{"schema":"hgtran-ai.rdd-mode-override/v1","mode":"off"}`+"\n")
	writeStoreResetFile(t, legacy, `{"schema":"hgtran-ai.rdd-mode-override/v1","mode":"off"}`+"\n")
...
// hgtran-ai directory just because someone asked what a reset would do.
...
//
// reviews/ is written by the hgtran-pi adapter, and neither guarantee this
