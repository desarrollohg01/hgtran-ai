...
const CompactAbandonAuthorizationSchema = "hgtran-ai.review-abandon-authorization/v2"

const compactLegacyAbandonAuthorizationSchema = "hgtran-ai.review-abandon-authorization/v1"
...
func compactAbandonV2Rerun(repo, lineage, revision, snapshotIdentity, actor, reason string, discarded CompactDiscardedWorkSummary) string {
	return fmt.Sprintf("Re-run `hgtran-ai review abandon --cwd %q --lineage %q --expected-revision %q --reason %q --actor %q --maintainer-authorization <the exact binding below>` using schema %s:\n%s",
