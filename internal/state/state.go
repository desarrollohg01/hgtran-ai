...
type InstallState struct {
	...
	// `hgtran-ai sync` preserves the user's model choices instead of falling
	...
	// (low|medium|high|xhigh). Persisted so that `hgtran-ai sync` preserves the
	...
	// (e.g. "gpt-5.6-sol", "gpt-5.6-luna"). Persisted so that `hgtran-ai sync`
	...
	// PendingSync is set to true when a hgtran-ai self-upgrade succeeded and
