...
func (adapter *CodexAdapter) Review(ctx context.Context, invocation Invocation) ([]byte, error) {
	...
	scratch, err := os.MkdirTemp("", "hgtran-ai-codex-reviewer-*")
	if err != nil {
...
func codexReviewerArguments(scratch, outputPath string) ([]string, error) {
	...
	return append(arguments,
		...
		"--config", fmt.Sprintf(`model_providers.%s={name="Hgtran AI reviewer loopback",base_url=%q,wire_api="responses"}`, codexReviewerLoopbackProviderID, baseURL),
	), nil
}
