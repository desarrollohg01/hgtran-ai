...
const (
	...
	// OwnershipMarker is embedded in every launcher written by Hgtran AI.
	// It is intentionally stable so deactivation can refuse to remove user files.
	OwnershipMarker = "hgtran-ai:managed-opencode-launcher/v1"
...
// BinDir returns the Hgtran-owned launcher directory.
func BinDir(homeDir string) string { return filepath.Join(homeDir, ".hgtran-ai", "bin") }
...
// they are asking for Hgtran-owned paths.
...
// Hgtran-owned bin directory. It must be called before that directory is added
