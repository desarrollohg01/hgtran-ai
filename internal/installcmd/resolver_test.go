...
func TestBashScriptPathWindowsUsesForwardSlashes(t *testing.T) {
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
	got := bashScriptPath(profile, `C:\Users\jorge\AppData\Local\Temp\hgtran-guardian-angel\install.sh`)
	want := "C:/Users/jorge/AppData/Local/Temp/hgtran-guardian-angel/install.sh"
	if got != want {
...
func TestResolveComponentInstall(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:      CommandSequence{{"brew", "tap", "desarrollohg01/homebrew-tap"}, {"brew", "install", "engram"}},
		},
		...
		},
		{
			...
			want:      CommandSequence{{"brew", "tap", "desarrollohg01/homebrew-tap"}, {"brew", "reinstall", "hga"}},
		},
		{
			...
			want: CommandSequence{
				...
				{"git", "-C", "/tmp/hgtran-guardian-angel", "fetch", "--depth=1", "https://github.com/Gentleman-Programming/hgtran-guardian-angel.git", "refs/tags/v" + versions.GGAVersion + ":refs/tags/v" + versions.GGAVersion},
		...
		},
		{
			...
			component: model.ComponentGGA,
			want: CommandSequence{
				{"git", "clone", "--depth=1", "--branch", "v" + versions.GGAVersion, "https://github.com/Gentleman-Programming/hgtran-guardian-angel.git", filepath.Join(os.TempDir(), "hgtran-guardian-angel")},
				{gitBashPath(), bashScriptPath(system.PlatformProfile{OS: "windows"}, filepath.Join(os.TempDir(), "hgtran-guardian-angel", "install.sh"))},
			},
		},
		{
...
func TestResolveGGAInstall_UsesPinnedReleaseTag(t *testing.T) {
	...
	wantFetch := []string{
		"git",
		"-C",
		"/tmp/hgtran-guardian-angel",
		"fetch",
		"--depth=1",
		"https://github.com/Gentleman-Programming/hgtran-guardian-angel.git",
		tagRef + ":" + tagRef,
	}
...
func TestGGAInstall_CleanupCommandBehavior(t *testing.T) {
	...
	tmpDir := t.TempDir()
	staleDir := filepath.Join(tmpDir, "hgtran-guardian-angel")
	...
	if profile.OS == "windows" {
		...
		systemTemp := system.PowerShellSingleQuoted(filepath.Join(os.TempDir(), "hgtran-guardian-angel"))
		...
		// Cleanup command: rm -rf /tmp/hgtran-guardian-angel
