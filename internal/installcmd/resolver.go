...
// resolveClaudeCodeInstall returns the npm install command sequence hgtran-ai
// shows for Claude Code — display text only, never executed by hgtran-ai
...
// a pin only guarded against a tampered "latest" tag when hgtran-ai itself
...
// resolveKilocodeInstall returns the npm install command sequence hgtran-ai
// shows for Kilocode — display text only, never executed by hgtran-ai. On
...
func validatePiInstallPreflight() error {
	if _, err := cmdLookPath("pi"); err != nil {
		return fmt.Errorf("Pi requires the `pi` executable in PATH before installing Hgtran AI Pi packages")
	}
...
// hgtran-ai shows for OpenCode per platform — never executed by hgtran-ai.
...
func resolveGGAInstall(profile system.PlatformProfile) (CommandSequence, error) {
	...
	default:
		...
		if profile.OS == "linux" && profile.PackageManager != "" {
			const tmpDir = "/tmp/hgtran-guardian-angel"
