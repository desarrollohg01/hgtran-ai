...
if [[ "$1" == api && "$2" == "repos/$GITHUB_REPOSITORY/releases/tags/$tag" ]]; then
  cat <<JSON
{"tag_name":"$tag","draft":false,"prerelease":false,"assets":[{"name":"hgtran-ai_1.2.3_darwin_amd64.tar.gz"},{"name":"hgtran-ai_1.2.3_darwin_arm64.tar.gz"},{"name":"hgtran-ai_1.2.3_linux_amd64.tar.gz"},{"name":"hgtran-ai_1.2.3_linux_arm64.tar.gz"},{"name":"hgtran-ai-review-provider-contract-1.1.0.tar.gz"},{"name":"checksums.txt"},{"name":"checksums.txt.minisig"}]}
JSON
  exit 0
fi
if [[ "$1" == release && "$2" == download && "$3" == "$tag" ]]; then
  ...
  printf 'provider contract\n' >"$directory/hgtran-ai-review-provider-contract-1.1.0.tar.gz"
  (cd "$directory" && sha256sum hgtran-ai_1.2.3_*.tar.gz hgtran-ai-review-provider-contract-1.1.0.tar.gz >checksums.txt)
...
func TestGoReleaserSignsBoundManifestAndInjectsTrustAnchors(t *testing.T) {
	...
	for _, required := range []string{
		...
		`github.com/hgtran-programming/hgtran-ai/v2/internal/update/upgrade.releaseMinisignPublicKeys={{ .Env.MINISIGN_PUBLIC_KEYS_CANONICAL }}`,
		"-trimpath",
		...
		"hgtran-ai-release-provenance-v1",
...
func TestCanonicalReleasePublicKeysControlRealLinkerBuild(t *testing.T) {
	...
	const linkerTarget = "github.com/hgtran-programming/hgtran-ai/v2/internal/update/upgrade.releaseMinisignPublicKeys"
