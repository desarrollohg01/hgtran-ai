...
func TestAtomicReplaceFailsWhenTheBinaryIsNotReplaced(t *testing.T) {
	dir := t.TempDir()
	src := filepath.Join(dir, "hgtran-ai.new")
	dst := filepath.Join(dir, "hgtran-ai")
...
func TestAtomicReplacePublishesTheStagedBinary(t *testing.T) {
	...
	assertNoLeftovers(t, dir, "hgtran-ai")
}
...
func TestWriteExecutableLeavesNoBinaryWhenTheCopyFails(t *testing.T) {
	dir := t.TempDir()
	outPath := filepath.Join(dir, "hgtran-ai.new")
	...
	assertNoLeftovers(t, dir, "hgtran-ai.new")
}
...
func TestWriteExecutablePublishesAnExecutable(t *testing.T) {
	dir := t.TempDir()
	outPath := filepath.Join(dir, "nested", "hgtran-ai.new")
	content := []byte("#!/bin/sh\necho hgtran-ai\n")
