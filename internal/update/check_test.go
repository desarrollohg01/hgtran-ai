...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "hgtran-ai uses build var",
			tool:         ToolInfo{Name: "hgtran-ai", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "hgtran-ai dev build",
			tool:         ToolInfo{Name: "hgtran-ai", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestUsesBetaMainHeadCheck(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tool:           ToolInfo{Name: "engram", Owner: "desarrollohg01", Repo: "engram"},
			currentVersion: "1.40.3-0.20260614211459-b6872c69e3e4",
			want:           false,
		},
	}
...
func TestFetchLatestReleaseMatchingPatternSkipsPiChannel(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/desarrollohg01/engram/releases" {
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "hgtran-ai-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "hgtran-ai-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	// hgtran-ai: 1.5.0 local == 1.5.0 remote → UpToDate
	assertResult(t, results[0], "hgtran-ai", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// hgtran-ai has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("hgtran-ai status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("hgtran-ai expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "hgtran-ai macOS brew-owned",
			tool:          ToolInfo{Name: "hgtran-ai"},
			profile:       system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			brewInstalled: true,
			want:          "brew upgrade hgtran-ai",
		},
		{
			name:    "hgtran-ai macOS non-brew",
			tool:    ToolInfo{Name: "hgtran-ai"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "hgtran-ai upgrade (downloads pre-built binary)",
		},
		{
		...
		},
		{
			...
			want:    "Windows binary distribution and Scoop are temporarily unavailable until publicly trusted Authenticode signing is enforced. Install/update from source with Go 1.25.10+: go install bitbucket.org/hgt_development/hgtran-ai/v2/cmd/hgtran-ai@latest",
		},
		{
...
func TestHomebrewPackageInstalledWithRequiresActiveBrewPath(t *testing.T) {
	...
	if homebrewPackageInstalledWith(func(string, ...string) *exec.Cmd { return mockCmd("true") }, func(string) (string, error) { return "", fmt.Errorf("not found") }, "hgtran-ai") {
...
func TestRegistryContents(t *testing.T) {
	...
	expected := map[string]struct {
		...
		// The three tools HG forked resolve to HG. TestNoForkedToolPointsAtTheUpstream
		// in fork_identity_test.go guards the same invariant from the other side.
	...
	// hgtran-ai must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("hgtran-ai DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (hgtran-ai) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just hgtran-ai to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("hgtran-ai dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes hgtran-ai to be reported as DevBuild.
...
// when the running hgtran-ai binary reports version "dev", it is identified as a
// DevBuild and NOT reported as UpdateAvailable or VersionUnknown.
//
// The spec says:
//   - Dev build MUST be reported as development-build semantic
//   - hgtran-ai self-upgrade is skipped while engram/gga remain eligible
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return mockCmd("false") }
	Tools = []ToolInfo{Tools[0]} // hgtran-ai only
	...
	r := results[0]
	if r.Tool.Name != "hgtran-ai" {
		t.Fatalf("tool = %q, want hgtran-ai", r.Tool.Name)
	}
...
// hgtran-ai with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only hgtran-ai and engram for this test
	...
	// hgtran-ai should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("hgtran-ai status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip hgtran-ai to avoid dev-build behavior)
...
func TestDetectInstalledVersionPs1FallbackInvokesViaPowershell(t *testing.T) {
	tmpDir := t.TempDir()
	ps1Path := filepath.Join(tmpDir, "hga.ps1")
