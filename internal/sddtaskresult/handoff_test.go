...
func TestHandoffCarriesThePrefixAndSchema(t *testing.T) {
	...
	for field, want := range map[string]any{
		...
		"phase":        "sdd-apply",
		"continuation": "hgtran-ai sdd-status --cwd '/repo' --json",
	} {
...
func TestHandoffContinuationNamesTheChangeWhenKnown(t *testing.T) {
	for _, tt := range []struct{ change, want string }{
		{change: "", want: "hgtran-ai sdd-status --cwd '/repo' --json"},
		{change: "feat-x", want: "hgtran-ai sdd-status 'feat-x' --cwd '/repo' --json"},
	} {
		var decoded map[string]any
