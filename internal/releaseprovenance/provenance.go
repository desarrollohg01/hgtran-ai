...
const (
	schema = "hgtran-ai.release-provenance/v1"
...
func Build(config []byte, input Input) ([]byte, error) {
	...
	for _, platform := range platforms {
		artifacts = append(artifacts, binaryArtifact{
			Name: "hgtran-ai_" + version + "_" + platform[0] + "_" + platform[1] + ".tar.gz", Kind: "binary",
			GOOS: platform[0], GOARCH: platform[1], CGOEnabled: "0", Trimpath: true,
		})
	}
	artifacts = append(artifacts, contractArtifact{Name: "hgtran-ai-review-provider-contract-" + input.ProviderContractSemver + ".tar.gz", Kind: providerArchiveKind})
