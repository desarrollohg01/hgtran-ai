...
const (
	...
	reviewResultDryRunSchema       = "hgtran-ai.review-capture-result-dry-run/v1"
...
func RunReviewCaptureResult(args []string, stdout io.Writer) error {
	...
	if flags.NArg() != 0 || strings.TrimSpace(*lineage) == "" || strings.TrimSpace(*target) == "" ||
		...
		return reviewPreflightError(errors.New("review capture-result requires an exact repository context, --lineage, --target, --lens, --order, and either --input or --agent (or --preflight); `hgtran-ai review status --contract hgtran-ai.review-integration/v1 --next-transition` prints the exact bindings and `hgtran-ai review schema reviewer` emits the result schema with a working example"))
	}
	...
	if *preflight && strings.TrimSpace(*input) == "" {
		if *subjectHash != "" && *subjectHash != subject.SubjectHash {
			...
			if legacyErr != nil {
				return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("review capture preflight subject hash does not match the provider-owned authority; refresh the binding with hgtran-ai review status --cwd <repo> --contract <same-contract> --next-transition"))
			}
	...
	if providerExecution {
		...
		if *materialize {
			if *subjectHash != "" && *subjectHash != subject.SubjectHash {
				return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("materialize subject hash does not match the provider-owned authority; refresh the binding with hgtran-ai review status --cwd <repo> --contract <same-contract> --next-transition"))
			}
