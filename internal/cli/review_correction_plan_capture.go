...
func RunReviewCaptureCorrectionPlan(args []string, stdout io.Writer) error {
	...
	if record.State.CapturePhaseRevision != strings.TrimSpace(*revision) || record.State.State != reviewtransaction.StateCorrectionRequired ||
		...
		return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("correction-plan capture binding does not match the current correction authority; rerun `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition` before retrying"))
	}
	...
	if request.RequestHash != strings.TrimSpace(*requestHash) || request.TargetIdentity != strings.TrimSpace(*target) {
		return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("correction-plan capture request does not match current authority; rerun `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition` before retrying"))
	}
	state := record.State
