...
const (
	...
	// transport, but with the launcher owned by hgtran-pi: the Pi host reads
...
// The Pi launcher lives in hgtran-pi and is versioned independently; it
...
const reviewPiHostRelayContract = "hgtran-pi.review-relay/v1"
...
func reviewTransportRefusalExitGuidance() string {
	return "; exit receipt-driven review with `hgtran-ai review mode disable --scope clone --cwd <repo>`; supported immutable review runtimes: " +
...
// `GENTLE_PI_REVIEW_RELAY_CONTRACT=hgtran-pi.review-relay/v1` renders as
// `<redacted>` and `hgtran-pi.review-relay/v1` renders as
// `hgtran-pi.review-relay<redacted>`. Naming the variable alone survives the
...
func reviewPiRelayHandshakeGuidance() string {
	return "; pi is eligible only while " + reviewPiHostRelayContractEnvironment +
		" declares the exact relay contract this binary admits, which the hgtran-pi host exports on every invocation it relays; export it in this shell and re-run"
}
