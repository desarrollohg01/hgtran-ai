...
func TestRunSDDVerifyValidateHelpIsSuccessfulAndInputFree(t *testing.T) {
	...
	for _, want := range []string{"Usage: hgtran-ai sdd-verify-validate", "--input <path|->", "--requirements <n>", "--scenarios <n>"} {
...
func TestRunSDDVerifyValidateHelpRespectsFlagValueArity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantHelp {
				if err != nil || !strings.Contains(output.String(), "Usage: hgtran-ai sdd-verify-validate") {
