...
func runReviewLensContext(args []string, help io.Writer, deps reviewLensContextDeps) (payload []byte, err error) {
	...
	if flags.NArg() != 0 || strings.TrimSpace(*repositoryContext) == "" || strings.TrimSpace(*lens) == "" ||
		...
		return nil, reviewPreflightError(errors.New("review lens-context requires the exact provider-issued repository context, lineage, target, expected revision, and lens carried by the collect transition; run `hgtran-ai review lens-context --help` for the closed command form"))
	}
	...
	if !reviewtransaction.ReviewerContextLevelAccepted(level) {
		return nil, reviewPreflightError(fmt.Errorf("unknown reviewer context delivery %q; run `hgtran-ai review lens-context --help` for the closed command form", *delivery))
	}
...
func reviewLensContextInstructionText(binding reviewLensContextBinding, paths int) (string, error) {
	...
	return fmt.Sprintf(`You are the %s lens of one bounded Hgtran AI review. %s
