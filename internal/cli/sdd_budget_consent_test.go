...
func TestExhaustedBudgetAsksInsteadOfDeadEnding(t *testing.T) {
	...
	for _, want := range []string{"hgtran-ai sdd-attempt reset", "--cwd", "--change", "--expected-revision", "sha256:" + strings.Repeat("a", 64)} {
...
func TestExhaustedBudgetSurfacesTheQuestionEndToEnd(t *testing.T) {
	...
	if !strings.Contains(scoped.Consent.Choices[0].Invocation, "hgtran-ai sdd-attempt reset") ||
