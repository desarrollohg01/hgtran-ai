...
func TestReviewQuarantineLegacyFixScopeContract(t *testing.T) {
	...
	for _, want := range []string{"complete_fix_scope_expansion", "validate_fix_alias", "quarantine-historical-complete-fix-scope-expansion", "hgtran-ai.review-legacy-fix-scope-quarantine-authorization/v2, repository, lineage, revision, diagnostic, disposition, anomaly_set, validate_fix_alias_revision, validate_fix_alias_operation, actor, reason"} {
