	...
	*remediatesEvidenceRevision = strings.TrimSpace(*remediatesEvidenceRevision)
	...
	if operation == "begin" || (operation == "acquire" && (*token == "" || declaredUntracked) && !skipAcquireUntrackedPreflight) {
		...
		if !inheritsRescopeSelection {
			scope, scopeErr := intendedUntrackedScopeForTarget(ctx, reviewtransaction.SnapshotBuilder{Repo: *cwd}, untrackedScope, intendedUntracked, expectedUntrackedInventory, reviewIntendedUntrackedInventoryCommand, "hgtran-ai sdd-attempt "+operation)
			if scopeErr != nil {
				return scopeErr
			}
			if scope.NeedsSelection {
				return intendedUntrackedSelectionRequired(scope, reviewIntendedUntrackedInventoryCommand, "hgtran-ai sdd-attempt "+operation)
			}
...
func missingSDDAttemptOperationError(operation string, missing []string) error {
	...
	case "acquire", "settle", "handoff", "grant":
		message += fmt.Sprintf("; rerun `hgtran-ai sdd-attempt %s` with those missing flags", operation)
	}
