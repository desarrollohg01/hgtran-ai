...
func (capture reviewProviderCapture) continuation() string {
	return fmt.Sprintf("hgtran-ai review status --cwd <repo> --contract %s --agent %s --lineage %s --next-transition", ReviewIntegrationContractV2, capture.runtime, capture.state.LineageID)
}
