...
func storeResetSeedLineage(t *testing.T, repo, lineage string, state reviewtransaction.State) {
	...
	payload := `{"schema":"hgtran-ai.review-state-record/v2","revision":"sha256:00","state":{"schema":"hgtran-ai.review-state/v2","lineage_id":"` +
...
func TestReviewStoreResetWithdrawsTheUntouchedPromiseWhenARestoreFailed(t *testing.T) {
	report := reviewtransaction.StoreResetReport{
		...
		Repository: "/repo", StoreRoot: "/repo/.git/hgtran-ai",
	...
	for _, want := range []string{
		...
		"/repo/.git/hgtran-ai/.store-reset-42/worktrees-view-a",
	} {
...
// at the surface the user types. reviews/ is written by the hgtran-pi adapter,
