...
func (err *reviewModeRepositoryRequiredError) Error() string {
	return "clone-local review mode requires a Git repository; rerun the original command with --cwd pointing at the intended repository, or use `hgtran-ai review mode enable --scope global` or `hgtran-ai review mode disable --scope global` for machine-wide state"
}
...
func emitReviewMode(stdout io.Writer, result ReviewModeResult, emitJSON bool) error {
	...
	if result.Operation == "enable" && result.Scope == reviewModeScopeClone &&
		...
		if _, err = fmt.Fprint(
			stdout,
			"  note:        a clone-local override can only disable, so this cleared the clone's off opinion and the global switch still decides; run `hgtran-ai review mode enable --scope global` to turn receipt-driven development on\n",
		); err != nil {
			return err
		}
	}
	...
	// working kill switch to someone half of whose hgtran-ai installations are
	// still enforcing review.
	_, err = fmt.Fprint(
		stdout,
		"  note:        applied for this hgtran-ai only; a hgtran-ai installed before the switch moved reads a location this command could not open, and keeps enforcing the value it holds there\n",
	)
	return err
}
...
func authorizeReviewStart(ctx context.Context, repo string, assessment reviewtransaction.RiskAssessment, consent reviewStartConsentMode, negotiated bool) error {
	...
	if negotiated && !console.Interactive {
		...
		// writes zero bytes to stderr (hgtran-pi fails closed on any stderr a
