...
const reviewProviderRoleCaptureSchema = "hgtran-ai.review-provider-role-capture/v1"
...
func parseReviewProviderRoleCapture(command string, args []string, stdout io.Writer, withRequestHash bool) (*reviewProviderRoleCaptureBinding, error) {
	...
	if flags.NArg() != 0 || binding.lineage == "" || binding.target == "" || binding.revision == "" ||
		...
		return nil, reviewPreflightError(fmt.Errorf("review %s requires --lineage, --target, --expected-revision, --agent, and either --materialize or --execute; `hgtran-ai review status --contract %s --next-transition` prints the exact bindings", command, ReviewIntegrationContractV2))
	}
	if binding.runtime == "" {
	...
	if reviewProviderCaptureRuntime(binding.runtime) && binding.materialize {
		return nil, reviewPreflightError(fmt.Errorf("review %s --materialize is unavailable for %q: its compiled Go adapter executes the provider contract directly; rerun `hgtran-ai review %s` with the same binding and --execute", command, binding.runtime, command))
	}
...
func (binding *reviewProviderRoleCaptureBinding) discover(ctx context.Context) (reviewtransaction.CompactStore, reviewtransaction.CompactRecord, error) {
	...
	if record.State.LineageID != binding.lineage || record.State.CapturePhaseRevision != binding.revision {
		return store, record, reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, fmt.Errorf("review %s binding does not match the current compact authority; refresh the binding with hgtran-ai review status --cwd <repo> --contract %s --next-transition", binding.command, ReviewIntegrationContractV2))
	}
...
func RunReviewCaptureRefuter(args []string, stdout io.Writer) error {
	...
	if state.State != reviewtransaction.StateReviewing || state.InitialSnapshot.Identity != binding.target {
		return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("review capture-refuter requires the exact reviewing authority target; refresh the binding with hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition"))
	}
...
func RunReviewCaptureValidation(args []string, stdout io.Writer) error {
	...
	if request.ValidationRequest.CorrectionTargetIdentity != binding.target {
		return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("review capture-validation target does not match the frozen correction target identity; refresh the binding with hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition"))
	}
	if request.ValidationRequest.RequestHash != binding.requestHash {
		return reviewPreflightRefusal(reviewPreflightCaptureBindingMismatchReason, errors.New("review capture-validation request hash does not match the frozen targeted validation request; refresh the binding with hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition"))
	}
	if binding.materialize {
