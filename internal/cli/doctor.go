...
import (
	...
	"github.com/desarrollohg01/hgtran-ai/v2/internal/storage"
)
...
func checkStateJSON(homeDir string) CheckResult {
		...
		if dir := agentConfigDir(homeDir, agentID); dir != "" {
			...
			if os.IsNotExist(lstatErr) {
				...
				if ancestorErr != nil {
					return CheckResult{
						Name:   id,
						Status: CheckStatusWarn,
						Detail: fmt.Sprintf("managed config path %s could not be inspected: %v; inspect or repair it manually, then re-run 'hgtran-ai doctor'", dir, ancestorErr),
					}
				}
				if ancestor != "" {
			...
			if lstatErr != nil {
				return CheckResult{
					Name:   id,
					Status: CheckStatusWarn,
					Detail: fmt.Sprintf("managed config path %s could not be inspected: %v; inspect or repair it manually, then re-run 'hgtran-ai doctor'", dir, lstatErr),
				}
			}
			if info.Mode()&os.ModeSymlink != 0 {
				if _, statErr := os.Stat(dir); os.IsNotExist(statErr) {
					...
					return CheckResult{Name: id, Status: CheckStatusWarn, Detail: fmt.Sprintf("managed config symlink target %s could not be inspected: %v; inspect or repair it manually, then re-run 'hgtran-ai doctor'", dir, statErr)}
				}
			}
		}
	}
	...
	if len(dangling) > 0 {
		detail := fmt.Sprintf("state lists %d agent(s) whose managed config paths are dangling symlinks: %s; inspect or repair these paths manually, then re-run 'hgtran-ai doctor'", len(dangling), strings.Join(dangling, ", "))
		if len(missing) > 0 {
...
// at or above homeDir are never inspected: they are not managed by hgtran-ai,
...
func danglingAncestor(homeDir, path string) (string, error) {
	...
	for ancestor := filepath.Dir(filepath.Clean(path)); ancestor != home && strings.HasPrefix(ancestor, boundary); ancestor = filepath.Dir(ancestor) {
		...
		if info.Mode()&os.ModeSymlink == 0 {
			...
			return "", fmt.Errorf("ancestor %s is not a directory", ancestor) // refusal:by-design world-action: the caller embeds this cause in a warn that already names the continuation (inspect or repair the path, re-run 'hgtran-ai doctor'); the repair itself happens on the filesystem, not through a command
		}
...
func checkEngramReachable(ctx context.Context, homeDir string, installedAgents []string) CheckResult {
	...
	if err != nil {
		return CheckResult{
			...
			Remedy: doctor.NewRemedy(doctor.RemedyInspectEngram, "Repair the persisted Engram MCP configuration, then run 'hgtran-ai sync'"),
		}
	}
	if len(commands) == 0 {
		return CheckResult{
			...
			Remedy: doctor.NewRemedy(doctor.RemedySync, "Run 'hgtran-ai sync' to restore the Engram MCP configuration"),
		}
	}
...
func checkInstalledAssetVersion(homeDir string) CheckResult {
	...
	if s.InstalledBinaryVersion != AppVersion {
		return CheckResult{
			Status: CheckStatusWarn,
			Detail: fmt.Sprintf("installed assets were configured by hgtran-ai %s, but running binary is %s — run 'hgtran-ai sync' to update installed assets", s.InstalledBinaryVersion, AppVersion),
		}
	}
	return CheckResult{
		Status: CheckStatusPass,
