...
// is nothing more to do from here" are wrong: `hgtran-ai review mode
...
func TestReviewNarrationNeverClaimsNothingMoreToDo(t *testing.T) {
	...
	for code, statement := range reviewStopReasonNarration {
		...
		if strings.Contains(lowered, bannedClaim) {
			t.Errorf("stop reason %q narration still claims %q, which is false: `hgtran-ai review mode disable` is always reachable: %q", code, bannedClaim, statement)
		}
	}
}
...
// name a real `hgtran-ai` command / `--flag` of its own, or fall back to the
// universal self-service exit `hgtran-ai review mode disable`
...
func TestReviewNarrationNamesAUniversalOrBetterExit(t *testing.T) {
	...
	for code, statement := range reviewStopReasonNarration {
		...
		if !namesOtherContinuation.MatchString(statement) {
			t.Errorf("stop reason %q narration names no runnable `hgtran-ai` command, no `--flag`, and no %q fallback, so this stop reads as a dead end: %q", code, reviewConsentOffPathCommand, statement)
		}
	}
}
...
// `hgtran-ai review mode disable` would let an orchestrator silently
...
// .git/hgtran-ai directory.
func TestReviewNarrationReviewModeDisableIsAlwaysCloneScoped(t *testing.T) {
	re := regexp.MustCompile("`hgtran-ai review mode disable[^`]*`")
	found := 0
	for code, statement := range reviewStopReasonNarration {
		for _, invocation := range re.FindAllString(statement, -1) {
			...
			if !strings.Contains(invocation, "--scope clone") || !strings.Contains(invocation, "--cwd <repo>") {
				t.Errorf("stop reason %q: %s defaults to global scope if run as printed (verified by execution: omitting --scope writes ~/.hgtran-ai/state.json machine-wide) -- name --scope clone --cwd <repo> instead", code, invocation)
			}
		}
	}
	if found == 0 {
		t.Fatal("found no `hgtran-ai review mode disable` invocations in reviewStopReasonNarration; the extraction is stale")
	}
}
...
// review_narration.go: `hgtran-ai review status --next-transition` alone is
// refused by this real CLI without `--contract hgtran-ai.review-integration/v2
// --agent claude-code`, and `hgtran-ai review reopen-results` is refused
...
func TestReviewNarrationNamedCommandsAreAlwaysComplete(t *testing.T) {
	statusRe := regexp.MustCompile("`hgtran-ai review status[^`]*--next-transition`")
	reopenRe := regexp.MustCompile("`hgtran-ai review reopen-results[^`]*`")
	for code, statement := range reviewStopReasonNarration {
		for _, invocation := range statusRe.FindAllString(statement, -1) {
			...
			if !strings.Contains(invocation, "--contract hgtran-ai.review-integration/v2") || !reviewAgentBindingRegexp.MatchString(invocation) {
				t.Errorf("stop reason %q: %s is incomplete -- the real CLI refuses --next-transition without --contract hgtran-ai.review-integration/v2 and a bound --agent", code, invocation)
			}
		}
