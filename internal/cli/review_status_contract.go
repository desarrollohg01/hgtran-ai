...
const ReviewIntegrationStatusSchemaV4 = "hgtran-ai.review-integration.status/v4"
const ReviewIntegrationStatusSchemaIDV4 = "https://hgtran-ai.dev/contracts/review-integration/v2/schemas/status-v4.schema.json"
const ReviewIntegrationStatusSchemaV5 = "hgtran-ai.review-integration.status/v5"
const ReviewIntegrationStatusSchemaIDV5 = "https://hgtran-ai.dev/contracts/review-integration/v2/schemas/status-v5.schema.json"
const ReviewIntegrationStatusSchemaV6 = "hgtran-ai.review-integration.status/v6"
const ReviewIntegrationStatusSchemaIDV6 = "https://hgtran-ai.dev/contracts/review-integration/v2/schemas/status-v6.schema.json"
...
func (result ReviewTargetStatusResult) validateNextTransitionTargets() error {
	...
	if result.Applicability == reviewtransaction.TargetApplicabilityUnrelated {
		...
		if result.Projection.Kind == reviewtransaction.TargetCurrentChanges && len(result.Projection.Paths) == 0 {
			...
			if input.Name != "base_ref" || input.Schema != "hgtran-ai.review-base-ref-selection/v1" ||
...
func (result ReviewTargetStatusResult) validateIntendedUntrackedSelectionTransition() error {
	if result.NextTransition.Collect == nil || len(result.NextTransition.Collect.Inputs) != 1 {
		return errors.New("fresh target lacks an intended-untracked selection transition; rerun `hgtran-ai review status --next-transition`")
	}
	input := result.NextTransition.Collect.Inputs[0]
	...
	case reviewNextTransitionCollect:
		...
		for _, input := range transition.Collect.Inputs {
			...
			if input.CaptureOperation == reviewCaptureCorrectionPlanOperation &&
				(input.Schema != "hgtran-ai.review-correction-plan/v1" || len(arguments) != 5 ||
					strings.TrimSpace(arguments["lineage"]) == "" || !validReviewCapabilitySHA256(arguments["expected-revision"]) ||
