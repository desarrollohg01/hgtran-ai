...
func TestCheckStateJSON_ManagedConfigPathUnreadable(t *testing.T) {
	...
	wants := []string{configPath, "inspect or repair", "hgtran-ai doctor"}
	if runtime.GOOS == "windows" {
...
func setupDanglingOpenCodeFixture(t *testing.T, statePayload, symlinkTarget string) (homeDir, configDir, targetPath string) {
	...
	if err := os.MkdirAll(filepath.Join(homeDir, ".hgtran-ai"), 0o755); err != nil {
		t.Fatal(err)
	}
	if symlinkTarget == "" {
	...
	if err := os.WriteFile(filepath.Join(homeDir, ".hgtran-ai", "state.json"), []byte(statePayload), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestCheckStateJSON_AgentConfigDirDanglingSymlink(t *testing.T) {
	...
	if !strings.Contains(got.Detail, "inspect") || !strings.Contains(got.Detail, "hgtran-ai doctor") {
		t.Fatalf("detail must provide manual inspection and doctor rerun guidance, got %q", got.Detail)
	}
	if strings.Contains(got.Detail, "hgtran-ai sync") {
...
func TestCheckStateJSON_DanglingAndAbsentConfigDirsSuppressSync(t *testing.T) {
	...
	for _, want := range []string{configDir, "genuinely absent config dirs: claude-code", "inspect or repair", "hgtran-ai doctor"} {
		if !strings.Contains(got.Detail, want) {
...
func TestCheckStateJSON_DanglingAncestorSymlinkSuppressesSync(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(homeDir, ".hgtran-ai", "state.json"), []byte(`{"installed_agents":["opencode"]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{configDir, "dangling ancestor symlink " + ancestor, "inspect", "hgtran-ai doctor"} {
		if !strings.Contains(got.Detail, want) {
			t.Fatalf("dangling ancestor result missing %q: %s", want, got.Detail)
		}
	}
	if strings.Contains(got.Detail, "hgtran-ai sync") || strings.Contains(got.Detail, "config dirs are missing") {
...
func TestCheckStateJSON_AbsentAncestorChainKeepsSyncRemedy(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(homeDir, ".hgtran-ai", "state.json"), []byte(`{"installed_agents":["windsurf"]}`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestCheckInstalledAssetVersion_SkewWarning(t *testing.T) {
	...
	if !strings.Contains(got.Detail, "v0.9.0") || !strings.Contains(got.Detail, "hgtran-ai sync") {
		t.Errorf("unexpected detail: %s", got.Detail)
	}
}
...
// #2078: a default install. hgtran-ai only ever configures Engram as a stdio
...
Status:  healthy
`, configPath, filepath.Join(homeDir, ".hgtran-ai"))
...
func TestRunDoctor_DanglingConfigSymlinkIsReadOnly(t *testing.T) {
	...
	if strings.Contains(output.String(), "hgtran-ai sync") {
		t.Fatalf("Doctor must not recommend sync for a dangling config symlink, got:\n%s", output.String())
	}
	for _, want := range []string{configDir, "inspect", "hgtran-ai doctor"} {
