...
const reviewAcknowledgedSchema = "hgtran-ai.review-acknowledged/v1"
	...
	if len(claims) > 0 {
		...
		if errors.Is(readErr, errReviewProviderRefuterResultNotCaptured) {
			...
			if _, captured, err := reviewProviderCaptureRefuter(ctx, repo, store, state, state.CapturePhaseRevision, runtime); err != nil {
				...
				return nil, errors.New("compiled provider refuter was required but no result was captured; rerun `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition` and follow its capture route")
			}
