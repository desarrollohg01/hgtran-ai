...
func TestEveryReviewStopReasonCodeHasAShippedContinuation(t *testing.T) {
	...
	const disableCommand = "`D` means `hgtran-ai review mode disable --scope clone --cwd <B>`"
	...
	namesOtherContinuation := regexp.MustCompile("`hgtran-ai [a-z][a-z-]*|`--[a-z][a-z-]*")
	for code, continuation := range assetCodes {
		...
		t.Errorf("shipped %s entry for %q names no runnable `hgtran-ai` command, no `--flag` to pass on the same invocation, and no D/S continuation alias, so this stop reads as a dead end", reviewLedgerContractAsset, code)
	}
	...
	// rdd_disabled.
	if continuation := assetCodes["rdd_disabled"]; !strings.Contains(continuation, "`hgtran-ai review mode enable --scope global`") {
		t.Errorf("shipped %s entry for rdd_disabled does not name the command that enables (`hgtran-ai review mode enable --scope global`): %q", reviewLedgerContractAsset, continuation)
	}
}
...
// `hgtran-ai review status ... --next-transition` invocation.
var reviewStatusNextTransitionInvocationRegexp = regexp.MustCompile("`hgtran-ai review status[^`]*--next-transition`")

// TestNamedReviewStatusNextTransitionIsAlwaysComplete is the execution-based
// RED-first proof for adversarial finding F1: `hgtran-ai review status
// --next-transition` alone is refused by this real CLI --
//
//	Error: --action-eligibility and --next-transition require --contract hgtran-ai.review-integration/v1
//
...
// routing from. The only form that actually runs is `hgtran-ai review
// status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent
...
func TestNamedReviewStatusNextTransitionIsAlwaysComplete(t *testing.T) {
	for label, content := range reviewStopReasonDocsCompleteDocuments(t) {
		for _, invocation := range reviewStatusNextTransitionInvocationRegexp.FindAllString(content, -1) {
			if !strings.Contains(invocation, "--contract hgtran-ai.review-integration/v2") || !reviewAgentBindingRegexp.MatchString(invocation) {
				t.Errorf("%s: %s is incomplete -- the real CLI refuses --next-transition without --contract hgtran-ai.review-integration/v2 and a bound --agent (verified by execution)", label, invocation)
			}
		}
	}
}
...
// `hgtran-ai review reopen-results ...` invocation.
var reviewReopenResultsInvocationRegexp = regexp.MustCompile("`hgtran-ai review reopen-results[^`]*`")

// TestNamedReviewReopenResultsIsAlwaysComplete is the execution-based
// RED-first proof for adversarial finding F7: `hgtran-ai review
...
func TestNamedReviewReopenResultsIsAlwaysComplete(t *testing.T) {
	const bareNominalReference = "`hgtran-ai review reopen-results`"
	...
	for label, content := range reviewStopReasonDocsCompleteDocuments(t) {
		for _, invocation := range reviewReopenResultsInvocationRegexp.FindAllString(content, -1) {
			...
			// ("`hgtran-ai review reopen-results` is a bounded maintenance
...
// reviewModeDisableInvocationRegexp matches any backtick-quoted `hgtran-ai
// review mode disable ...` invocation.
var reviewModeDisableInvocationRegexp = regexp.MustCompile("`hgtran-ai review mode disable[^`]*`")
...
// `global` (review_mode.go's own flag default), so `hgtran-ai review mode
...
// Verified by execution: the bare form writes ~/.hgtran-ai/state.json;
// `--scope clone --cwd <B>` writes only under the selected target root's
// .git/hgtran-ai directory. Every invocation in the shipped asset must name
...
func TestNamedReviewModeDisableIsAlwaysCloneScoped(t *testing.T) {
	...
	if len(invocations) == 0 {
		t.Fatal("found no `hgtran-ai review mode disable` invocations in the shipped asset; the extraction is stale")
	}
	for _, invocation := range invocations {
		if !strings.Contains(invocation, "--scope clone") || !strings.Contains(invocation, "--cwd <B>") {
			t.Errorf("shipped asset: %s defaults to global scope if run as printed (verified by execution: omitting --scope writes ~/.hgtran-ai/state.json machine-wide) -- name --scope clone --cwd <B> instead", invocation)
		}
	}
}
