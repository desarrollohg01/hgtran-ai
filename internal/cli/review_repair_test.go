...
// issue #3409. `hgtran-ai review repair --preflight` exists so a maintainer
...
func TestReviewRepairPreflightNamesAWayForwardWhenTheStoreExceedsTheBound(t *testing.T) {
	repo := initReviewCLIRepo(t)
	compactRoot := filepath.Join(repo, ".git", "hgtran-ai", "review-transactions", "v2")
	...
	if !strings.Contains(output.String(), "hgtran-ai review inspect-authority") {
