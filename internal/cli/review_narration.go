...
// naming the bare `hgtran-ai review mode disable` would let a reader
...
// bare form writes ~/.hgtran-ai/state.json; this scoped form writes only
// under the named repository's own .git/hgtran-ai directory.
const reviewModeDisableCloneCommand = "hgtran-ai review mode disable --scope clone --cwd <repo>"
...
var reviewStopReasonNarration = map[string]string{
	...
	"corrected_candidate_unavailable": "Change the candidate content so it differs from the frozen original, then re-run " +
		"`hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent " + reviewUndeclaredRuntimeIdentitySlot + " --next-transition`. " +
		...
		"reopen their lenses over the same frozen content: run `hgtran-ai review reopen-results --prepare --cwd <repo> --lineage <id> " +
	...
	"empty_base_diff_bootstrap_required": "This selected committed base has no changes to review. " +
		...
		"Then run `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --agent " + reviewUndeclaredRuntimeIdentitySlot + " --next-transition --base-ref <empty-root> --committed-only`.",
	"lens_context_budget_exceeded": "This frozen candidate cannot fit complete reviewer evidence without truncation, so this review stops before an inspection result. " +
		"Reduce the candidate scope or target identity, then run `hgtran-ai review start` for that new candidate; or run `" + reviewModeDisableCloneCommand + "` " + reviewModeDisableCloneCaveat + " to deliver under ordinary repository policy instead.",
	...
	"rdd_disabled": "Review mode is disabled. Run `hgtran-ai review mode status --cwd <repo> --json` to inspect the deciding scope; STATUS renders the exact scoped enable command for this request.",
...
// successful negotiated operation is machine-readable end to end (hgtran-pi
