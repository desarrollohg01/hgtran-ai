...
// operator reads.
//
// The clone-scope switch is machine state: a modern hgtran-ai publishes it
// under the relocated switch root, and every hgtran-ai installed before that
...
func TestCloneScopeDisableSaysWhenItReachedOnlyThisBuild(t *testing.T) {
	...
	if !strings.Contains(output.String(), "note:") ||
		!strings.Contains(output.String(), "applied for this hgtran-ai only") {
