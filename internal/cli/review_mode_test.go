...
func TestReviewModeCloneScopeEnableMigratesLegacyRevision(t *testing.T) {
	...
	legacyRoot := filepath.Join(repo, ".git", "hgtran-ai", "review-transactions")
	...
	if err := os.Rename(filepath.Join(repo, ".git", "hgtran-ai", "review-mode"), legacyRoot); err != nil {
		t.Fatalf("relocate secure legacy fixture: %v", err)
	}
	if _, err := os.Lstat(filepath.Join(repo, ".git", "hgtran-ai", "review-mode")); !errors.Is(err, os.ErrNotExist) {
...
// the same explicit global "on" that `hgtran-ai review mode enable` persists,
...
// "default"` and stays byte-for-byte the same shape, because hgtran-pi decodes
...
func TestReviewModeCloneScopeEnableNamesTheGlobalExitWhileGlobalUnset(t *testing.T) {
	...
	for _, want := range []string{
		...
		"can only disable",
		"hgtran-ai review mode enable --scope global",
	} {
	...
	for key := range envelope.Status {
		...
		default:
			t.Fatalf("status envelope grew a field %q; hgtran-pi decodes hgtran-ai.rdd-mode-status/v1 as an exact object", key)
		}
	}
