...
// hgtran-ai.opencode-review-provider-role/v1 envelope for a captured provider
...
func openCodeProviderRoleResultEnvelope(role reviewProviderRole) string {
	return `{"schema":"hgtran-ai.opencode-review-provider-role/v1","role":"` + string(role) + `","captured":true}`
}
...
func openCodeTransportFailure(code string) error {
	return fmt.Errorf("%s: OpenCode Task transport did not produce a capturable reviewer result; run `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition` before retrying", code)
}

func openCodeTransportAuthorityUnavailable(cause error) error {
	return fmt.Errorf("opencode_review_transport_authority_unavailable: OpenCode Task transport did not produce a capturable reviewer result; run `hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition` before retrying: %w", cause)
}
