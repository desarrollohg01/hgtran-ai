...
// <GitCommonDir>/hgtran-ai/rejected-results/<lineage>/, outside the working
...
const (
	reviewRejectedResultSchema  = "hgtran-ai.review-rejected-result/v1"
...
func writeReviewRejectedResult(ctx context.Context, root string, meta reviewRejectedResultMeta, raw []byte) (string, error) {
	...
	dir := filepath.Join(lease.Identity().GitCommonDir, "hgtran-ai", reviewRejectedResultDirName, reviewRejectedResultPathComponent(meta.LineageID))
