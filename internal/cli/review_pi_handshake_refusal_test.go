...
// nothing else.
//
// The old cause offered `hgtran-ai review mode disable --scope clone` and
...
const piRelayHandshakeRefusalCause = "the active runtime is not eligible for immutable receipt review; " +
	...
	"which the hgtran-pi host exports on every invocation it relays; export it in this shell and re-run"
...
func TestPiRelayHandshakeGuidanceSurvivesTheFailureCausePrivacyGate(t *testing.T) {
	...
	if scrubbed := reviewScrubDefectReportField("the value " + reviewPiHostRelayContract); scrubbed != "the value hgtran-pi.review-relay"+reviewDefectReportRedactionMarker {
...
func TestPiHandshakeGuidanceStaysScopedToPi(t *testing.T) {
	...
	for _, runtime := range []string{string(model.AgentKilocode), "unknown-runtime"} {
		t.Run(runtime, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "hgtran-ai review mode disable --scope clone --cwd <repo>") {
...
func TestPiRelayHandshakeIsSoleMissingConditionStaysDiagnostic(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "pi with a stale contract", declared: "hgtran-pi.review-relay/v0", agent: model.AgentPi, want: true},
