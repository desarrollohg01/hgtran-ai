...
func TestUnsupportedImmutableReviewTransportStopsBeforeRepositoryOrAuthority(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "logical orchestrator role", runtime: "hgtran-orchestrator", startCode: reviewTransportCapabilityUnsupportedCode},
		//
	...
	for _, args := range [][]string{
		...
		{"start", "--contract", ReviewIntegrationContractV2, "--agent", "hgtran-orchestrator", "--target", target, "--projection", "workspace", "--cwd", repo},
...
func TestImmutableReviewTransportRefusalNamesWorkingExits(t *testing.T) {
	...
	for _, runtime := range []model.AgentID{model.AgentKilocode} {
		t.Run(string(runtime), func(t *testing.T) {
			...
			const exit = "hgtran-ai review mode disable --scope clone --cwd <repo>"
...
func TestPiHostRelayContractHandshakeGatesAdmission(t *testing.T) {
	for _, declared := range []string{"", "hgtran-pi.review-relay/v0", "HGTRAN-PI.REVIEW-RELAY/V1"} {
