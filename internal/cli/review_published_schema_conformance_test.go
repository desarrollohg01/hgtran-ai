...
func compileWholePublishedReviewSchema(t *testing.T, version, name string) *jsonschema.Schema {
	...
	for _, resourceVersion := range versions {
		...
		for _, entry := range entries {
			...
			location := "https://hgtran-ai.dev/contracts/review-integration/" + resourceVersion + "/schemas/" + entry.Name()
			if err := compiler.AddResource(location, document); err != nil {
				t.Fatal(err)
			}
		}
	}
	schema, err := compiler.Compile("https://hgtran-ai.dev/contracts/review-integration/" + version + "/schemas/" + name)
	if err != nil {
		t.Fatal(err)
	}
	return schema
