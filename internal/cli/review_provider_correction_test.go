...
func rejectedResultsDir(t *testing.T, repo, lineage string) string {
	...
	return filepath.Join(lease.Identity().GitCommonDir, "hgtran-ai", reviewRejectedResultDirName, lineage)
}
...
func TestProviderCaptureRefusesAfterTwoRejectedResultsAndPreservesBoth(t *testing.T) {
	...
	for _, want := range []string{`unknown field "lens"`, "no complete JSON object", "hgtran-ai review status"} {
