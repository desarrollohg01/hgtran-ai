...
const piBackgroundPolicySchema = "hgtran-pi.background-subagents/v1"
...
// piBackgroundPolicyPath resolves the hgtran-pi-readable policy location:
// <base>/hgtran-ai/background-subagents.json where base is
...
func piBackgroundPolicyPath(homeDir string) string {
	...
	return filepath.Join(base, "hgtran-ai", "background-subagents.json")
}

// piBackgroundProjectionPlan writes the RESOLVED effective on/off policy to
// the hgtran-pi-readable location. Auto never reaches projection: it resolves
