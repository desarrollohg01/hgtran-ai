...
func TestSettleRefusesReusedAcquireRequestIDAndNamesTheExit(t *testing.T) {
	...
	assertExitNames(t, reused.Exit, "distinct --request-id", "unit-1", "`hgtran-ai sdd-attempt status --cwd <repo> --change <change>`")
...
func TestSettleNamesMalformedTokenWithoutMentioningFinish(t *testing.T) {
	...
	assertExitNames(t, err.Error(), "--token", "`hgtran-ai sdd-attempt status --cwd <repo> --change <change>`", "`hgtran-ai sdd-attempt settle`")
}
...
func TestCompleteNamesTheSuccessorWorkUnit(t *testing.T) {
	...
	assertExitNames(t, repeated.Exit, "different label", "slice-1", "`hgtran-ai sdd-attempt acquire --cwd <repo> --change <change>")
	...
	assertExitNames(t, err.Error(), "different --work-unit", "`hgtran-ai sdd-attempt acquire --cwd <repo> --change <change>")
