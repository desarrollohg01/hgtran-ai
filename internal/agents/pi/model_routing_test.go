...
func TestResolvePackageBinForms(t *testing.T) {
	const valid, malformed, manifestBound = `{"bin":{"hgtran-pi-models":"bin/hgtran-pi-models"}}`, `{"bin":`, MaxPackageManifestBytes
	...
	cases := []struct{ name, manifest, target, link, kind string }{
		{"string", `{"name":"hgtran-pi-models","bin":"bin/hgtran-pi-models"}`, "bin/hgtran-pi-models", "", ""}, {"object and canonical symlink", `{"bin":{"hgtran-pi-models":"bin/hgtran-pi-models"}}`, "real/hgtran-pi-models", "symlink", ""},
		{"exact bound", valid + strings.Repeat(" ", manifestBound-len(valid)), "bin/hgtran-pi-models", "", ""}, {"bound plus one", valid + strings.Repeat(" ", manifestBound+1-len(valid)), "bin/hgtran-pi-models", "", "manifest-too-large"}, {"malformed within bound", malformed + strings.Repeat(" ", manifestBound-len(malformed)), "bin/hgtran-pi-models", "", "malformed-manifest"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			want = canonicalWant
			if tc.link != "" {
				bin := filepath.Join(root, "bin", "hgtran-pi-models")
...
func TestResolvePackageBinErrors(t *testing.T) {
	cases := []struct{ name, manifest, domain, kind, setup string }{
		...
		{"string bin with another package name", `{"name":"other-package","bin":"bin/hgtran-pi-models"}`, "bin", "absent-bin", ""}, {"malformed manifest", `{"bin":`, "manifest", "malformed-manifest", ""},
		{"malformed bin", `{"bin":true}`, "bin", "malformed-bin", ""}, {"absent bin", `{"name":"hgtran-pi-models"}`, "bin", "absent-bin", ""},
		{"absent object bin", `{"bin":{"other":"bin/other"}}`, "bin", "absent-bin", ""}, {"missing target", `{"bin":{"hgtran-pi-models":"bin/missing"}}`, "bin", "missing-bin-target", ""},
		{"non-regular target", `{"bin":{"hgtran-pi-models":"bin/hgtran-pi-models"}}`, "bin", "non-regular-bin-target", "directory"}, {"non-executable target", `{"bin":{"hgtran-pi-models":"bin/hgtran-pi-models"}}`, "bin", "non-executable-bin-target", "nonexec"},
		{"absolute target", `{"bin":{"hgtran-pi-models":"/outside"}}`, "bin", "unsafe-bin", ""}, {"lexical escape", `{"bin":{"hgtran-pi-models":"../outside"}}`, "bin", "unsafe-bin", ""},
		{"duplicate top-level bin", `{"bin":"bin/x","bin":"bin/y"}`, "bin", "malformed-bin", ""}, {"duplicate selected bin", `{"bin":{"hgtran-pi-models":"bin/x","hgtran-pi-models":"bin/y"}}`, "bin", "malformed-bin", ""}, {"symlink escape", `{"bin":{"hgtran-pi-models":"bin/hgtran-pi-models"}}`, "bin", "unsafe-bin", "symlink"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case "directory":
				must(t, os.MkdirAll(filepath.Join(root, "bin", "hgtran-pi-models"), 0o755))
			case "nonexec":
				writeTarget(t, root, "bin/hgtran-pi-models", 0o644)
			case "symlink":
