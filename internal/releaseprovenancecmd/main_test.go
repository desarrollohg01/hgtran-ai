...
func ciEnvironment(t *testing.T) {
	...
	t.Setenv("GITHUB_REPOSITORY", "Gentleman-Programming/hgtran-ai")
...
func TestLocalBuildWritesNoReleaseClaim(t *testing.T) {
	...
	if body["schema"] != "hgtran-ai.release-provenance/local-build" {
	...
	for _, missing := range releaseIdentityEnvironment {
		t.Run("without "+missing, func(t *testing.T) {
			...
			if wantProvenance {
				...
				if !strings.Contains(string(payload), `"schema":"hgtran-ai.release-provenance/v1"`) {
...
func TestCIBuildStillProducesReleaseProvenance(t *testing.T) {
	...
	t.Run("another repository still refuses", func(t *testing.T) {
		out, args := provenanceArgs(t)
		ciEnvironment(t)
		t.Setenv("GITHUB_REPOSITORY", "someone-else/hgtran-ai")
		if err := run(args); err == nil {
