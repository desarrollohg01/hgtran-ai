...
func TestAllEmbeddedAssetsAreReadable(t *testing.T) {
	expectedFiles := []string{
		...
		"claude/persona-hgtran.md",
		"claude/sdd-orchestrator.md",
		"claude/commands/hgtran-sdd-apply.md",
		"claude/commands/hgtran-sdd-archive.md",
		"claude/commands/hgtran-sdd-continue.md",
		"claude/commands/hgtran-sdd-explore.md",
		"claude/commands/hgtran-sdd-ff.md",
		"claude/commands/hgtran-sdd-init.md",
		"claude/commands/hgtran-sdd-new.md",
		"claude/commands/hgtran-sdd-onboard.md",
		"claude/commands/hgtran-sdd-research.md",
		"claude/commands/hgtran-sdd-status.md",
		"claude/commands/hgtran-sdd-verify.md",
		...
		// Kimi agent files
		"kimi/persona-hgtran.md",
		"kimi/output-style-hgtran.md",
		"kimi/output-style-neutral.md",
		"kimi/sdd-orchestrator.md",
		"kimi/KIMI.md",
		"kimi/agents/hgtran.yaml",
		...
		"skills/hgtran-ai-bench/SKILL.md",
	}
...
func TestSDDVerifyAdmissionPrecedesPersistence(t *testing.T) {
	...
	for _, path := range []string{"claude/agents/sdd-verify.md", "claude/commands/hgtran-sdd-verify.md", "cursor/agents/sdd-verify.md", "kimi/agents/sdd-verify.md", "kiro/agents/sdd-verify.md"} {
		content := MustRead(path)
...
func TestOpenCodeEmbeddedAssetLayout(t *testing.T) {
	...
	for _, name := range []string{"commands", "plugins", "persona-hgtran.md", "background-subagents.md", "sdd-orchestrator.md", "sdd-overlay-single.json", "sdd-overlay-multi.json"} {
		if !seen[name] {
...
func TestOpenCodeBackgroundPolicyMarkersAreBalanced(t *testing.T) {
	...
	const (
		start = "<!-- hgtran-ai:opencode-background-subagents -->"
		end   = "<!-- /hgtran-ai:opencode-background-subagents -->"
	)
...
func TestOpenCodeReviewTransportPluginContract(t *testing.T) {
	...
	for _, want := range []string{`hgtran-ai.provider-transport/v1`, `"review", "opencode-transport"`, `RELAY_REGISTRY_KEY`, `reviewRelayRegistry()`, `output.args.prompt = (await relay.prompt).prompt`, `output.output = await registration.relay.complete(output.output)`, `"tool.execute.before"`, `"tool.execute.after"`,
...
func TestSkillRegistryPluginContract(t *testing.T) {
	...
	// stdout belongs to OpenCode commands whose output hgtran-ai parses
...
func TestClaudeEmbeddedAssetLayout(t *testing.T) {
	...
	for _, name := range []string{"agents", "commands", "persona-hgtran.md", "sdd-orchestrator.md"} {
		if !seen[name] {
...
func TestOpenCodeSDDOrchestratorDelegationVisibility(t *testing.T) {
	...
	for _, required := range []string{
		"<!-- hgtran-ai:opencode-desktop-delegation-progress -->",
...
func TestClaudeSDDWorkflowRequiresSessionPreflight(t *testing.T) {
	...
	for _, required := range []string{
		...
		"If any dependency is missing, STOP and propose `/hgtran-sdd-new` or `/hgtran-sdd-ff`; do not implement",
...
func TestOpenCodeSDDCommandsAreOrchestratorGuarded(t *testing.T) {
	...
	for _, required := range []string{
		"You are the `hgtran-orchestrator`, not an SDD executor",
...
func TestGentlemanLanguageInstructionsDoNotBiasEnglishSessions(t *testing.T) {
	...
	personaPaths := []struct {
		...
		{path: "claude/persona-hgtran.md", combineWith: "claude/output-style-hgtran.md", languagePhrase: "Always match the user's current language in your reply."},
		{path: "generic/persona-hgtran.md", languagePhrase: "Match the user's current language in your REPLY ONLY"},
		{path: "kiro/persona-hgtran.md", languagePhrase: "Match the user's current language in your REPLY ONLY"},
		{path: "kimi/persona-hgtran.md", combineWith: "kimi/output-style-hgtran.md", languagePhrase: "Always match the user's current language in your reply."},
		{path: "opencode/persona-hgtran.md", languagePhrase: "Match the user's current language in your REPLY ONLY"},
	}
...
func TestClaudeManagedOutputStylesAnchorReplyLanguageToLatestUserRequest(t *testing.T) {
	tests := []struct {
		...
		{
			path: "claude/output-style-hgtran.md",
			artifactContracts: []string{
...
func TestClaudeGentlemanPersonaPreventsEnglishGreetingCodeSwitching(t *testing.T) {
	...
	content := MustRead("claude/persona-hgtran.md") + "\n" + MustRead("claude/output-style-hgtran.md")
	...
	for _, required := range []string{
		...
		if !strings.Contains(content, required) {
			t.Fatalf("claude/persona-hgtran.md missing code-switching guardrail %q", required)
		}
	}
}
...
func TestPersonasContainContextualSkillLoadingDirective(t *testing.T) {
	tests := []struct {
		...
		{path: "claude/persona-hgtran.md", isClaude: true, invokeMsg: "invoke it via the built-in `Skill` tool"},
		{path: "opencode/persona-hgtran.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "generic/persona-hgtran.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "generic/persona-neutral.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "kiro/persona-hgtran.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
		{path: "kimi/persona-hgtran.md", isClaude: false, invokeMsg: "read the matching SKILL.md"},
	}
...
func TestOpenCodeSDDOverlaySubagentsAreExplicitExecutors(t *testing.T) {
	for _, assetPath := range []string{"opencode/sdd-overlay-single.json", "opencode/sdd-overlay-multi.json"} {
		t.Run(assetPath, func(t *testing.T) {
			...
			orchestrator, ok := agents["hgtran-orchestrator"].(map[string]any)
			if !ok {
				t.Fatalf("%q missing hgtran-orchestrator agent", assetPath)
			}
			permissions, ok := orchestrator["permission"].(map[string]any)
			if !ok || permissions["question"] != "allow" {
				t.Fatalf("%q hgtran-orchestrator must allow question permission", assetPath)
			}
...
func TestSDDOrchestratorsUseNativeRuntimeAttemptAuthority(t *testing.T) {
	...
	for _, path := range paths {
		...
		if strings.Contains(section, "--successor-lineage") {
			t.Fatalf("%s names --successor-lineage, which hgtran-ai sdd-attempt settle does not define", path)
		}
		last := -1
	...
	for _, assetPath := range []string{
		...
		t.Run(assetPath, func(t *testing.T) {
			...
			if assetPath == "opencode/sdd-orchestrator.md" {
				dedicatedAgent = "hgtran-orchestrator"
			}
