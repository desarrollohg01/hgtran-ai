...
`research.md` and `sdd/{change-name}/research` use `hgtran-ai.sdd-research/v1` and record:
...
`hgtran-ai.sdd-preproposal/v1` records the positive `revision`, exploration outcome/reference, research request and classes, admission and outcome, OpenSpec and Engram evidence references, product decisions (`pending | confirmed`), and `proposal_ready`.
