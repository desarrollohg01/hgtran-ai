...
Use `sdd/{change-name}/research` for exact `hgtran-ai.sdd-research/v1` bytes and `sdd/{change-name}/preproposal` for exact `hgtran-ai.sdd-preproposal/v1` bytes. In hybrid mode, neither topic is authoritative alone: compare its revision and bytes with OpenSpec before proposal admission.
