...
`
	...
	frames := "{\"schema\":\"hgtran-ai.provider-transport/v1\",\"operation\":\"start\",\"prompt\":\"Go must receive this original host prompt\"}\n{\"schema\":\"hgtran-ai.provider-transport/v1\",\"operation\":\"complete\",\"nonce\":\"nonce\",\"output\":\"untrusted reviewer output\"}\n"
...
const TRANSPORT = { Command: "hgtran-ai", Schema: "hgtran-ai.provider-transport/v1", Start: "start", Prompt: "prompt", Complete: "complete", Result: "result" }
...
lines.on("line", (line) => {
  ...
  if (!start) {
    ...
    process.stdout.write(JSON.stringify({ schema: "hgtran-ai.provider-transport/v1", operation: "prompt", nonce: "nonce", prompt }) + "\n")
    return
  }
...
func runOpenCodeTransportPluginHarness(t *testing.T, modules map[string]string, harness, relay string) (string, string) {
	...
	if err := os.WriteFile(filepath.Join(bin, "hgtran-ai"), []byte(relay), 0o700); err != nil {
		t.Fatal(err)
	}
