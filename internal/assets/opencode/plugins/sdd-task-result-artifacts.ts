...
// cwd, and `hgtran-ai sdd-status --cwd /` is refused on the Go side. A root is
...
function continuationCommand(cwd: string): string {
  return cwd === "" ? "hgtran-ai sdd-status --cwd <repo> --json" : `hgtran-ai sdd-status --cwd ${shellQuote(cwd)} --json`
}
...
function sddTaskFailure(phase: string, cwd: string, cause: unknown, metadata?: unknown): SDDTaskFailureError {
  ...
  const failure: SDDTaskFailure = {
    ...
    handoff: SDD_TASK_FAILURE_PREFIX + JSON.stringify({
      schemaName: "hgtran-ai.sdd-task-result-failure/v1",
      status: "blocked",
      code,
      phase,
