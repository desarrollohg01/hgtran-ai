# Hgtran AI — SDD Orchestrator Instructions

Bind this to the dedicated `hgtran-orchestrator` agent only. Do NOT apply it to executor phase agents such as `sdd-apply` or `sdd-verify`.
...
- Treat `agent.hgtran-orchestrator.model` as authoritative when it is set.
