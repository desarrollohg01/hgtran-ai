...
- `/hgtran-sdd-init` → initialize SDD context; detects stack and testing capabilities.
- `/hgtran-sdd-explore <topic>` → investigate an idea; no implementation.
- `/hgtran-sdd-status [change]` → read-only structured status.
- `/hgtran-sdd-apply [change]` → implement pending tasks in batches.
- `/hgtran-sdd-verify [change]` → validate implementation against specs/tasks.
- `/hgtran-sdd-archive [change]` → close a completed change.
- `/hgtran-sdd-onboard` → guided end-to-end walkthrough.

Meta-commands are handled by the orchestrator directly and do not appear in autocomplete:

- `/hgtran-sdd-new <change>` → run exploration then proposal.
- `/hgtran-sdd-continue [change]` → run the next dependency-ready phase.
- `/hgtran-sdd-ff <name>` → fast-forward proposal → specs → design → tasks.

### Native SDD Dispatcher Guard

Before routing, continuing, applying, verifying, or archiving an SDD change, invoke the native dispatcher (`hgtran-ai sdd-continue [change] --cwd <repo>` or `hgtran-ai sdd-status [change] --cwd <repo> --json --instructions`). It resolves the artifact store the workspace declares and reports it in `artifactStore`.
...
This applies to `/hgtran-sdd-new`, `/hgtran-sdd-ff`, `/hgtran-sdd-continue`, `/hgtran-sdd-explore`, `/hgtran-sdd-status`, `/hgtran-sdd-apply`, `/hgtran-sdd-verify`, `/hgtran-sdd-archive`, and natural-language equivalents such as "use SDD to add dark mode" / "do it with SDD".
...
For a new product/code change request that says to use SDD, start at preflight -> init guard -> explore/proposal (`/hgtran-sdd-new` equivalent). Never launch `sdd-apply` just because the user asked to implement a feature.
