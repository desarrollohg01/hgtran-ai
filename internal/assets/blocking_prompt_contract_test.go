...
func TestCoordinatorOrchestratorsCarryGentleAIProviderDefectHandoff(t *testing.T) {
	requirements := []struct {
		...
		{name: "captured provider decline", text: "exact captured provider-owned `choices[answer=\"declined\"].invocation` from the `hgtran-ai.review-integration.consent/v3` envelope"},
		...
		{name: "review mode disable prohibition", text: "Do not invoke `hgtran-ai review mode disable` at clone or global scope within this handoff."},
	...
	for _, path := range allPaths {
		t.Run(path, func(t *testing.T) {
			...
			for index, choice := range []string{
				"  1. **Report the Hgtran AI defect and continue**:",
			...
			for _, prohibited := range []string{"Resume only after an installed published fix", "latest version", "hgtran-" + "report"} {
			...
			if count := strings.Count(contract, "hgtran-ai review mode disable"); count != 1 {
...
func TestCoordinatorOrchestratorsCarrySDDEditAuthorityConsentRelay(t *testing.T) {
	requirements := []string{
		...
		"typed `hgtran-ai.sdd-integration.consent/v1` envelope",
