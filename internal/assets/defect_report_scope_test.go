...
// Hgtran AI provider or tool defect", and from the user's seat everything that
...
// be doing when it happened. A Hgtran AI workflow merely HOSTING a failure is
	...
	for _, path := range allSDDOrchestratorAssetPaths(t) {
		t.Run(path, func(t *testing.T) {
			...
			for _, required := range []string{
				...
				"a Hgtran AI invocation produced it",
			...
			if strings.Contains(content, "appears blocked by a Hgtran AI provider or tool defect") {
