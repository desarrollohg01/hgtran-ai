...
// under node with a fake `hgtran-ai` on PATH that records its argv. OpenCode
...
const logPath = process.env.HGTRAN_AI_RELAY_LOG
...
appendFileSync(process.env.HGTRAN_AI_RELAY_LOG, JSON.stringify(process.argv.slice(2)) + "\n")
`
