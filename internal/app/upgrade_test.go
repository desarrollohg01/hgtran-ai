...
// TestRunArgs_UpgradeDryRun verifies that `hgtran-ai upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `hgtran-ai upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `hgtran-ai upgrade engram` filters
...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	out := buf.String()
	// Output should only mention engram or no-upgrades, not hgtran-ai or gga.
	// This is a soft check since the tool may not be installed.
	if strings.Contains(out, "hgtran-ai") && !strings.Contains(out, "engram") {
		t.Errorf("filtering to engram should not show hgtran-ai in output; got: %s", out)
	}
}
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"hgtran-ai", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Gentleman-Programming/hgtran-ai/releases",
				},
			},
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "hgtran-ai", "[--]", "hga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
