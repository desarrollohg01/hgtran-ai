...
func TestRunArgsNoArgumentRequiresInteractiveStdinAndStdout(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range []string{"--version", "hgtran-ai update", "--help"} {
...
func TestBuiltBinaryClosedStdinRefusesBeforeBubbleTea(t *testing.T) {
	...
	binaryPath := filepath.Join(t.TempDir(), "hgtran-ai")
	build := exec.Command("go", "build", "-o", binaryPath, "./cmd/hgtran-ai")
	build.Dir = repoRoot
	if output, err := build.CombinedOutput(); err != nil {
		t.Fatalf("go build ./cmd/hgtran-ai: %v\n%s", err, output)
	}
