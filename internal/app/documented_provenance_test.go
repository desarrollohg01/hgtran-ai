...
func TestRenderedSurfaceNamesOnlyItsOwnRuntime(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if raw := len(rawAgentBindingRegexp.FindAllString(string(content), -1)); raw != found {
			t.Errorf("golden:%s carries %d --agent bindings but only %d sit inside extractable hgtran-ai invocations; the difference is invisible to this rule and must be moved into a documented invocation or removed", name, raw, found)
		}
	}
