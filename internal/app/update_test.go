...
func TestRunUpgrade_ForwardsParsedArgsOnceWithoutReparsing(t *testing.T) {
	...
	updateCheckFiltered = func(_ context.Context, _ string, _ system.PlatformProfile, filters []string) []update.UpdateResult {
		...
		return []update.UpdateResult{{Tool: update.ToolInfo{Name: "hgtran-ai"}, Status: update.UpToDate}}
	}
...
// `hgtran-ai doctor` command. The exact wording is part of the public contract
...
func TestPrintPostUpgradeDoctorAdvisory_OutputFormat(t *testing.T) {
	...
	if !strings.Contains(out, "hgtran-ai doctor") {
		t.Errorf("output must mention 'hgtran-ai doctor', got %q", out)
	}
...
// successful `hgtran-ai upgrade` of the hgtran-ai binary prints the doctor
...
func TestRunUpgrade_PrintsDoctorAdvisoryAfterGentleAIUpgrade(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "restart hgtran-ai") {
		t.Errorf("runUpgrade() output missing restart notice:\n%s", out)
	}
	if !strings.Contains(out, "Run 'hgtran-ai doctor' to verify ecosystem health after upgrade") {
	...
	restartIdx := strings.Index(out, "restart hgtran-ai")
	advisoryIdx := strings.Index(out, "hgtran-ai doctor")
...
func TestRunUpgrade_DryRunDoesNotPrintDoctorAdvisory(t *testing.T) {
	...
	if strings.Contains(buf.String(), "hgtran-ai doctor") {
		t.Fatalf("dry-run output must NOT mention 'hgtran-ai doctor' advisory:\n%s", buf.String())
	}
}

// TestRunUpgrade_NonGentleAIUpgradeDoesNotPrintDoctorAdvisory verifies that
// upgrading a tool other than hgtran-ai (e.g. engram, gga) does NOT trigger
// the doctor advisory. The advisory is gated on hgtran-ai specifically.
func TestRunUpgrade_NonGentleAIUpgradeDoesNotPrintDoctorAdvisory(t *testing.T) {
	...
	if strings.Contains(buf.String(), "ecosystem health after upgrade") {
		t.Fatalf("non-hgtran-ai upgrade must NOT print post-upgrade doctor advisory:\n%s", buf.String())
	}
}
