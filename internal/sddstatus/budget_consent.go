...
func BudgetConsentEnvelope(in BudgetConsentInput) (BudgetConsentResult, error) {
	...
	core := consentenvelope.Core{
		...
		Choices: []consentenvelope.Choice{
			{
				...
				Invocation: fmt.Sprintf(
					"hgtran-ai sdd-attempt reset --cwd %s --change %s --expected-revision %q --request-id %s --reason %q --actor %s",
			...
			},
			{
				...
				Invocation: fmt.Sprintf(
					"hgtran-ai sdd-attempt status --cwd %s --change %s",
		...
		OffPath: consentenvelope.OffPath{
			...
			Command: fmt.Sprintf("hgtran-ai review mode status --cwd %s", pathquote.Quote(in.Repo)),
		},
	}
...
const sddRuntimeAuditActor = "hgtran-ai-runtime"
...
func sddBudgetConsentResetRequestID(in BudgetConsentInput) string {
	sum := sha256.Sum256([]byte("hgtran-ai.sdd-budget-consent-reset/v1\x00" + in.Change + "\x00" + in.Revision))
