...
func TestRuntimeLedgerRescopeRecoversZeroDriftDeadlock(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "hgtran-ai sdd-attempt rescope") || strings.Contains(err.Error(), "hgtran-ai sdd-attempt reset") {
...
func TestRuntimeLedgerRescopeReplayRefusesForgedWidenedRecord(t *testing.T) {
	...
	forgedRecord := runtimeRecord{
		...
		RequestDigest: runtimeValueHash("hgtran-ai.sdd-runtime-rescope-request/v1", request),
...
func TestRuntimeLedgerWidenedRescopeRefusalNamesTheExhaustRoute(t *testing.T) {
	...
	for _, want := range []string{"1 remaining attempt", "decision-required", "hgtran-ai sdd-attempt reset"} {
...
func TestRuntimeLedgerCompleteObjectiveRefusalNamesTheSuccessor(t *testing.T) {
	...
	for _, want := range []string{"--work-unit", "advance", "hgtran-ai sdd-attempt reset"} {
