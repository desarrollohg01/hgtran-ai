...
//
//	hgtran-ai sdd-attempt grant --cwd <repo> --change <name> [--expected-revision <rev>] --root <path>... --actor <actor> --reason <reason> --request-id <id> --change-instance <token>
var sddConsentGrantInvocationShape = regexp.MustCompile(
	`^hgtran-ai sdd-attempt grant --cwd \S+ --change \S+( --expected-revision \S+)?( --root \S+)+ --actor \S+ --reason \S+ --request-id \S+ --change-instance \S+$`)
...
var sddConsentDeclineInvocationShape = regexp.MustCompile(
	`^hgtran-ai sdd-status \S+ --cwd \S+$`)

func TestSDDIntegrationConsentContractsArePinned(t *testing.T) {
	root := filepath.Join("..", "..", "contracts", "sdd-integration", "v1")
	want := map[string]string{
		"fixtures/consent.fixture.json": "2ff2e09db482324990c3878c99dcb65be26469e4dc97a5421d4a42a6d5d7e444",
		"schemas/consent.schema.json":   "5ef6cbf81364b522fb5b8b17eb7d1c5737ddb9dd766973baf24eacee4781823c",
	}
...
func TestSDDIntegrationConsentValidateRejectsIncompleteEnvelopes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wrong schema", mutate: func(result *SDDIntegrationConsentResult) { result.Schema = "hgtran-ai.review-integration.consent/v1" }},
		...
		{name: "grant invocation missing a root", mutate: func(result *SDDIntegrationConsentResult) {
			result.Choices[0].Invocation = "hgtran-ai sdd-attempt grant --cwd /workspace/planning --change multi-repo-rollout --root /workspace/service-a --actor maintainer --reason rollout --request-id grant-1 --change-instance sdd-1"
		}},
		{name: "grant invocation missing actor", mutate: func(result *SDDIntegrationConsentResult) {
			result.Choices[0].Invocation = "hgtran-ai sdd-attempt grant --cwd /workspace/planning --change multi-repo-rollout --root /workspace/service-a --root /workspace/service-b --reason rollout --request-id grant-1 --change-instance sdd-1"
		}},
		{name: "grant invocation missing request-id", mutate: func(result *SDDIntegrationConsentResult) {
			result.Choices[0].Invocation = "hgtran-ai sdd-attempt grant --cwd /workspace/planning --change multi-repo-rollout --root /workspace/service-a --root /workspace/service-b --actor maintainer --reason rollout --change-instance sdd-1"
		}},
		{name: "grant invocation missing change-instance", mutate: func(result *SDDIntegrationConsentResult) {
			result.Choices[0].Invocation = "hgtran-ai sdd-attempt grant --cwd /workspace/planning --change multi-repo-rollout --root /workspace/service-a --root /workspace/service-b --actor maintainer --reason rollout --request-id grant-1"
		}},
		{name: "decline invocation is not status re-entry", mutate: func(result *SDDIntegrationConsentResult) {
			result.Choices[1].Invocation = "hgtran-ai review status"
		}},
