...
func (store RuntimeStore) Acquire(ctx context.Context, request CompactAcquireRequest) (CompactAttemptResult, error) {
	...
	if request.RemediatesEvidenceRevision != "" && !runtimeRevisionPattern.MatchString(request.RemediatesEvidenceRevision) {
		return CompactAttemptResult{}, errors.New("remediates_evidence_revision must be sha256; rerun `hgtran-ai sdd-attempt acquire` with --remediates-evidence-revision sha256:<64-lowercase-hex>")
	}
...
func normalizeCompactSettleRequest(request CompactSettleRequest) error {
	if request.Outcome == AttemptInterrupted && request.EvidenceRevision != "" {
		return errors.New("interrupted evidence_revision must be empty; rerun `hgtran-ai sdd-attempt settle` without --evidence-revision")
	}
	...
	if !runtimeRevisionPattern.MatchString(request.Token) {
		return errors.New("token must be the exact sha256:<64-lowercase-hex> value acquire returned; run " + compactStatusCommand +
			" to read the live attempt's token, then rerun `hgtran-ai sdd-attempt settle` with --token <that value>")
	}
...
func compactBlockedExitText(reason CompactBlockReason, token string) string {
	...
	case CompactBlockCorruptAuthority:
		return "the attempt ledger for this work unit cannot be read as valid authority; run " +
			"`hgtran-ai sdd-attempt status --cwd <repo> --change <change>` to see what is readable, " +
	...
	case CompactBlockInvalidContinuation:
		return "this call does not continue the attempt currently on record; run " +
			"`hgtran-ai sdd-attempt status --cwd <repo> --change <change>` to see the live attempt and its " +
	...
	case CompactBlockMaintainerDecision:
		...
		return "this work unit's attempt or changed-line budget needs a maintainer decision; run " +
			"`hgtran-ai sdd-attempt status --cwd <repo> --change <change>` for the accounting, then have a " +
			"maintainer reset the objective with `hgtran-ai sdd-attempt reset --cwd <repo> --change <change> " +
	...
	case CompactBlockActiveAttempt:
		...
		// Only `hgtran-ai sdd-attempt status --cwd <repo> --change <change>`
		...
		return "a distinct attempt token " + token + " is already active for this work unit; run " +
			"`hgtran-ai sdd-attempt status --cwd <repo> --change <change>` to see it, then add `--token " + token +
...
const compactStatusCommand = "`hgtran-ai sdd-attempt status --cwd <repo> --change <change>`"

const compactResetCommand = "`hgtran-ai sdd-attempt reset --cwd <repo> --change <change> --expected-revision <the revision that status prints> " +
...
func compactAcquireCommand(workUnit string) string {
	return "`hgtran-ai sdd-attempt acquire --cwd <repo> --change <change> --request-id \"<unique-request-id>\" --work-unit \"" + workUnit +
