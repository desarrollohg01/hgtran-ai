...
type Status struct {
	...
	// carrying the typed hgtran-ai.sdd-integration.consent/v1 envelope whose
...
// "archived", no phase remains, and nothing is blocked (#4002; hgtran-pi#538
...
func validateStatusContract(contract string) error {
	...
	return fmt.Errorf("unsupported sdd-status contract %q. Start a fresh implementation state and rerun `hgtran-ai sdd-status --contract hgtran-ai.sdd-status/v2`.", contract)
}
		...
		case 0:
			if len(activeChanges) > 0 {
				...
				return blockedStatus(ArtifactStoreOpenSpec, workspaceRoot, nil, nil, "sdd-new", []string{
					...
					fmt.Sprintf(
						"Exploration-only directories are not active changes: %s. Run `hgtran-ai sdd-status <change-name> --cwd %s` to inspect one explicitly.",
...
func resolveWorkspaceRoot(options ResolveOptions) (string, error) {
	...
	if filepath.Dir(root) == root {
		return "", fmt.Errorf("workspace root %q is a filesystem root, which never holds an SDD project: whatever produced this call passed the wrong --cwd. Rerun it against the project: `hgtran-ai sdd-status --cwd \"<project-directory>\" --json`. If the change is Engram-backed, this dispatcher is blind to it and should not be called at all", root)
	}
	return root, nil
}
...
// `hgtran-ai sdd-status --cwd <cwd> --json` as its continuation, which lands
...
// emitted spelling must be `hgtran-ai sdd-status <change> --cwd <root>`. The
...
func ambiguousChangeSelectionReasons(subject, workspaceRoot string, changes []string) []string {
	...
	for _, change := range changes {
		reasons = append(reasons, fmt.Sprintf(
			"Run `hgtran-ai sdd-status %s --cwd %s` to continue with %s.",
