...
func engramWorkspaceWithArchivedChangesOnly(t *testing.T, changes ...string) string {
	t.Helper()
	t.Setenv("ENGRAM_PROJECT", "hgtran-ai")
	workspace := t.TempDir()
...
func TestEmptyWorkspaceStatusProjectsV2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(string(payload), `"schemaName":"hgtran-ai.sdd-status"`) {
			...
			if !strings.Contains(markdown, `"schemaName": "hgtran-ai.sdd-status"`) {
