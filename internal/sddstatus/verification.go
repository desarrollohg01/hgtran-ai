...
const verifyEnvelopeFenceRefusal = "missing valid hgtran-ai.verify-result/v1 envelope: the first non-empty line must be ```yaml (```yml and any letter case are admitted; ~~~ fences, untagged fences, and content before the fence are not); check the exact bytes with hgtran-ai sdd-verify-validate --input <path|-> --requirements <n> --scenarios <n>"
...
func remediationFenceContainsResult(lines []string) bool {
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "schema:") {
			...
			if schema == RemediationResultSchema || schema == "hgtran-ai.remediation-evidence/v1" {
				return true
			}
		}
	}
	...
	if err := json.Unmarshal([]byte(strings.Join(remediationFenceContents(lines), "\n")), &fields); err == nil {
		...
		return schema == RemediationResultSchema || schema == "hgtran-ai.remediation-evidence/v1"
	}
	return false
}
