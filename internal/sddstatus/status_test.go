...
// hands the caller `hgtran-ai sdd-status --cwd <cwd> --json` as its
...
func TestAmbiguousChangeSelectionNamesARunnableCommandPerChange(t *testing.T) {
	...
	for _, change := range []string{"first", "second"} {
		...
		want := "hgtran-ai sdd-status " + change + " --cwd " + root
...
func TestParseCommandArgs(t *testing.T) {
	...
	want := CommandArgs{ChangeName: "add-auth", CWD: "/tmp/repo", JSON: true, IncludeInstructions: true, Contract: "hgtran-ai.sdd-status/v2"}
	if got != want {
