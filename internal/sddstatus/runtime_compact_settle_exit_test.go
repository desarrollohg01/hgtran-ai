...
func TestSettleWithoutActiveAttemptNamesAcquire(t *testing.T) {
	...
	assertSettleBlockedExit(t, result, "nothing to settle", "`hgtran-ai sdd-attempt acquire --cwd <repo> --change <change>")
}
...
func TestCompleteExitNamesEachSuccessorPrecondition(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "budget exceeded", last: exceeded, workUnit: "slice-2", want: []string{"exceeded its changed-line budget", "`hgtran-ai sdd-attempt reset --cwd <repo> --change <change>"}},
		{name: "older binary", last: unbound, workUnit: "slice-2", want: []string{"no finish candidate identity", "older binary", "`hgtran-ai sdd-attempt reset --cwd <repo> --change <change>"}},
	} {
...
func assertSettleBlockedExit(t *testing.T, result CompactAttemptResult, wants ...string) {
	...
	for _, want := range append(wants, "`hgtran-ai sdd-attempt status --cwd <repo> --change <change>`") {
		if !strings.Contains(result.Exit, want) {
