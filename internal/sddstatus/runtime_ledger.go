...
var (
	...
	ErrRuntimeObjectiveDone = errors.New("SDD runtime objective is complete; it continues through a successor objective, so run " +
		"`hgtran-ai sdd-attempt acquire --cwd <repo> --change <change> --request-id \"<unique-request-id>\" --work-unit \"<a different label>\" " +
...
func OpenRuntimeStore(ctx context.Context, repo, change string) (RuntimeStore, error) {
	if !validRuntimeChange(change) {
		return RuntimeStore{}, fmt.Errorf("invalid SDD change name %q; want letters, digits, and single hyphens or underscores between them, at most 96 characters; run `hgtran-ai sdd-status --cwd <repo> --json` to read the resolved changeName", change)
	}
	...
	dir := runtimeChangeLedgerDir(filepath.Join(commonDir, "hgtran-ai", "sdd-runtime"), change)
...
func (err *RuntimeRepositoryRequiredError) Error() string {
	return fmt.Sprintf("the SDD runtime attempt ledger needs a Git repository because its authority lives in the Git common directory, and %s is not inside one; run `git init` in that workspace (or run from the repository that contains it), then rerun the same `hgtran-ai sdd-attempt` command", err.Workspace)
}
...
func runtimeChangeLedgerDir(base, change string) string {
	...
	digest := strings.TrimPrefix(runtimeValueHash("hgtran-ai.sdd-runtime-change-identity/v1", change), "sha256:")
...
func (store RuntimeStore) Begin(ctx context.Context, request BeginAttemptRequest) (RuntimeStatus, error) {
	...
	legacyDigest := ""
	if legacyRequest {
		legacyDigest = runtimeValueHash("hgtran-ai.sdd-runtime-begin-request/v1", legacyBeginAttemptRequest{
...
func (store RuntimeStore) Finish(ctx context.Context, request FinishAttemptRequest) (RuntimeStatus, error) {
	...
	return store.mutate(ctx, request.ExpectedRevision, request.RequestID, digest, func(replay runtimeReplay) (runtimeRecord, error) {
		...
		if request.Outcome == AttemptInterrupted && request.EvidenceRevision != "" {
			return runtimeRecord{}, errors.New("interrupted attempts must omit evidence_revision; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --outcome interrupted and without --evidence-revision")
		}
		...
		if evidenceRemediation {
			if !chainHasFailedEvidence {
				...
				return runtimeRecord{}, errors.New("this correction names failed verification " + request.RemediatesEvidenceRevision + ", but the attempt chain records no failed verification at all; run `hgtran-ai sdd-attempt status --cwd <repo> --change <change>` to read the chain, then settle without --remediates-evidence-revision if nothing is being repaired")
			}
		...
		if request.Outcome == AttemptPassed && chainHasFailedEvidence && !evidenceRemediation {
			return runtimeRecord{}, fmt.Errorf("passing correction for failed verification %q requires --remediates-evidence-revision %q; rerun `hgtran-ai sdd-attempt settle` with that flag", chainFailedEvidence, chainFailedEvidence)
		}
...
func (store RuntimeStore) settlementUntrackedSelection(ctx context.Context, active RuntimeAttempt, request FinishAttemptRequest) ([]string, string, error) {
	if active.EligibleUntrackedInventory == "" {
		...
		if request.IntendedUntracked != nil {
			return nil, "", fmt.Errorf("%w: this attempt began before settle-time untracked declarations existed, so it has no inventory to declare against; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` without --untracked-scope", ErrRuntimeUndeclaredUntracked)
		}
	...
	if request.ExpectedUntrackedInventory != digest {
		return nil, "", fmt.Errorf("%w: this declaration was made against untracked inventory %s but the workspace now holds %s; rerun `hgtran-ai review status --next-transition` for the current inventory, then rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --expected-untracked-inventory=%s", ErrRuntimeUndeclaredUntracked, request.ExpectedUntrackedInventory, digest, digest)
	}
	selection := *request.IntendedUntracked
	for _, path := range selection {
		if !slices.Contains(inventory, path) {
			return nil, "", fmt.Errorf("%w: intended-untracked path %q is not in the current eligible inventory; rerun `hgtran-ai review status --next-transition` to see what is eligible, then rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with only those paths", ErrRuntimeUndeclaredUntracked, path)
		}
	}
	...
	for _, path := range active.IntendedUntracked {
		if slices.Contains(inventory, path) && !slices.Contains(selection, path) {
			return nil, "", fmt.Errorf("%w: this attempt began with %q in its candidate, and a settlement cannot take it back out; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --intended-untracked=%s included", ErrRuntimeUndeclaredUntracked, path, path)
		}
	}
...
func runtimeBornDuringUntrackedRefusal(undecided []string, digest string) error {
	...
	return fmt.Errorf("%w: this attempt left eligible untracked files its candidate does not include, so settling now would record them as no change at all: %s%s; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --untracked-scope=select --intended-untracked=<repo-relative-path> --expected-untracked-inventory=%s to account them, or --untracked-scope=exclude --expected-untracked-inventory=%s to leave them out on the record", ErrRuntimeUndeclaredUntracked, strings.Join(listed, ", "), remainder, digest, digest)
}
...
func (store RuntimeStore) Handoff(ctx context.Context, request HandoffAttemptRequest) (RuntimeStatus, error) {
	...
	digest := runtimeValueHash("hgtran-ai.sdd-runtime-handoff-request/v1", request)
	return store.mutate(ctx, request.ExpectedRevision, request.RequestID, digest, func(replay runtimeReplay) (runtimeRecord, error) {
...
func (store RuntimeStore) runtimeRescopeWidenedRefusal(status RuntimeStatus, flag string, requested, allowed int) error {
	...
	return fmt.Errorf(
		"%w: received %s %d, the current objective allows %d. A wider successor scope is reached by finishing this objective rather than by rescoping it: spend its %d remaining attempt(s), and the run that exhausts them reaches decision-required, where `hgtran-ai sdd-attempt reset --cwd %q --change %q --expected-revision \"<revision-from-status>\" --request-id \"<unique-request-id>\" --reason \"<why-the-objective-changed>\" --actor \"<actor>\"` opens a fresh budget of any size",
...
func (store RuntimeStore) runtimeObjectiveCompleteRefusal(status RuntimeStatus) error {
	return fmt.Errorf(
		"%w: it passed within budget, so this change continues through a SUCCESSOR objective, not a repeat of this one — re-run this begin with a different --work-unit (everything else may stay as it is) and it is admitted as an advance that carries this objective's evidence forward. To discard this scope instead of succeeding it, run `hgtran-ai sdd-attempt reset --cwd %q --change %q --expected-revision %q --request-id \"<unique-request-id>\" --reason \"<why-the-objective-changed>\" --actor \"<actor>\"`",
...
func runtimeMissingWorktreeExit(active RuntimeAttempt, bound, cwd, change, token string) string {
	return fmt.Sprintf("attempt %d is bound to worktree %s, which no longer exists on disk, so its passed or failed evidence cannot be measured; "+
		"settle it as interrupted from any worktree of this repository with `hgtran-ai sdd-attempt settle --cwd %s --change %s --token %s "+
		...
		"the objective with `hgtran-ai sdd-attempt reset --cwd %s --change %s --expected-revision <the revision that status prints> "+
...
func runtimeMissingWorktreeInterruptedRecord(status RuntimeStatus, active RuntimeAttempt, request FinishAttemptRequest) (runtimeRecord, error) {
	if request.IntendedUntracked != nil {
		return runtimeRecord{}, fmt.Errorf("%w: the bound worktree no longer exists, so no untracked inventory can be declared against it; rerun `hgtran-ai sdd-attempt settle` with --outcome interrupted and without --untracked-scope", ErrRuntimeUndeclaredUntracked)
	}
...
func (store RuntimeStore) runtimeObjectiveChangeRefusal(ctx context.Context, status RuntimeStatus) error {
	...
	if store.runtimeObjectiveResetAdmissible(ctx, status) {
		return fmt.Errorf(
			"%w: reset the objective, then begin again — `hgtran-ai sdd-attempt reset --cwd %s --change %q --expected-revision %q --request-id \"<unique-request-id>\" --reason \"<why-the-objective-changed>\" --actor \"<actor>\"`; the reset publishes a new ledger revision, so take the begin's --expected-revision from `hgtran-ai sdd-attempt status --cwd %s --change %q` after it commits",
	...
	if store.runtimeObjectiveRescopeAdmissible(ctx, status) {
		objective := status.Objective
		return fmt.Errorf(
			"%w: this objective's candidate has not drifted, so resetting it is refused as an elective budget reset, but a maintainer may authorize a narrower successor scope instead — `hgtran-ai sdd-attempt rescope --cwd %s --change %q --expected-revision %q --request-id \"<unique-request-id>\" --work-unit \"<narrower-work-unit>\" --evidence-goal \"<narrower-evidence-goal>\" --max-attempts \"<n, at most %d>\" --max-changed-lines \"<n, at most %d>\" --reason \"<why-the-objective-is-narrowing>\" --actor \"<actor>\"`; rescope carries cumulative_attempts and cumulative_changed_lines forward unchanged and refuses any max_attempts or max_changed_lines above the current objective's %d and %d",
...
func (store RuntimeStore) Grant(ctx context.Context, request GrantRootsRequest) (RuntimeStatus, error) {
	...
	digest := runtimeValueHash("hgtran-ai.sdd-runtime-grant-request/v1", request)
...
func (store RuntimeStore) Rescope(ctx context.Context, request RescopeObjectiveRequest) (RuntimeStatus, error) {
	...
	digest := runtimeValueHash("hgtran-ai.sdd-runtime-rescope-request/v1", request)
	return store.mutate(ctx, request.ExpectedRevision, request.RequestID, digest, func(replay runtimeReplay) (runtimeRecord, error) {
		status := replay.Status
		if status.ActiveAttempt != nil {
			return runtimeRecord{}, store.runtimeAttemptActiveRefusal(*status.ActiveAttempt)
...
func (store RuntimeStore) RepairConsecutiveRescope(ctx context.Context, request RepairConsecutiveRescopeRequest) (RuntimeStatus, error) {
	...
	digest := runtimeValueHash("hgtran-ai.sdd-runtime-repair-consecutive-rescope-request/v1", request)
...
func validateRuntimeBeginEvent(record runtimeRecord) error {
	...
	if runtimeValueHash("hgtran-ai.sdd-runtime-begin-request/v1", request) != record.RequestDigest &&
		(event.IntendedUntracked != nil || runtimeValueHash("hgtran-ai.sdd-runtime-begin-request/v1", legacy) != record.RequestDigest) {
...
func validateRuntimeRecordShape(record runtimeRecord) error {
	...
	case runtimeOperationHandoff:
		...
		if runtimeValueHash("hgtran-ai.sdd-runtime-handoff-request/v1", request) != record.RequestDigest {
	...
	case runtimeOperationRescope:
		...
		if runtimeValueHash("hgtran-ai.sdd-runtime-rescope-request/v1", request) != record.RequestDigest {
	...
	case runtimeOperationRepairConsecutiveRescope:
		...
		if runtimeValueHash("hgtran-ai.sdd-runtime-repair-consecutive-rescope-request/v1", request) != record.RequestDigest {
	...
	case runtimeOperationGrant:
		...
		if runtimeValueHash("hgtran-ai.sdd-runtime-grant-request/v1", request) != record.RequestDigest {
...
func canonicalRuntimeIntendedUntracked(paths []string) ([]string, error) {
	...
	if len(canonical) > maximumRuntimeIntendedUntracked {
		return nil, fmt.Errorf("intended_untracked must name at most %d paths; rerun `hgtran-ai sdd-attempt acquire` or `hgtran-ai sdd-attempt begin` with an inventory-validated selection", maximumRuntimeIntendedUntracked)
	}
	...
	for index, path := range canonical {
		if validateRuntimeText(path, 4096) != nil || filepath.IsAbs(path) || filepath.ToSlash(filepath.Clean(path)) != path ||
			...
			return nil, errors.New("intended_untracked must be canonical unique repository-relative paths; rerun `hgtran-ai sdd-attempt acquire` or `hgtran-ai sdd-attempt begin` with the inventory-validated --untracked-scope and --intended-untracked flags")
		}
	}
...
func normalizeFinishAttemptRequest(request FinishAttemptRequest) (FinishAttemptRequest, error) {
	...
	if request.Outcome == AttemptInterrupted && request.EvidenceRevision != "" && !runtimeRevisionPattern.MatchString(request.EvidenceRevision) {
		return FinishAttemptRequest{}, errors.New("interrupted evidence_revision must be empty or a canonical legacy sha256 revision; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --outcome interrupted and without --evidence-revision")
	}
	if request.Outcome != AttemptInterrupted && !runtimeRevisionPattern.MatchString(request.EvidenceRevision) {
		return FinishAttemptRequest{}, fmt.Errorf(
			"evidence_revision must be sha256:<64-lowercase-hex> (%s); rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --evidence-revision sha256:<64-lowercase-hex>",
	...
	if (request.IntendedUntracked == nil) != (request.ExpectedUntrackedInventory == "") {
		return FinishAttemptRequest{}, errors.New("an untracked declaration needs both its selection and the inventory digest it was made against; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with --untracked-scope and --expected-untracked-inventory together")
	}
	if request.ExpectedUntrackedInventory != "" && !runtimeRevisionPattern.MatchString(request.ExpectedUntrackedInventory) {
		return FinishAttemptRequest{}, errors.New("expected_untracked_inventory must be sha256:<64-lowercase-hex>; rerun `hgtran-ai sdd-attempt finish` or `hgtran-ai sdd-attempt settle` with the digest `hgtran-ai review status --next-transition` publishes")
	}
	if request.IntendedUntracked != nil {
	...
	if request.RemediatesEvidenceRevision != "" {
		...
		if !runtimeRevisionPattern.MatchString(request.RemediatesEvidenceRevision) {
			return FinishAttemptRequest{}, fmt.Errorf(
				"remediates_evidence_revision must be sha256:<64-lowercase-hex> (%s); rerun `hgtran-ai sdd-attempt finish` with --remediates-evidence-revision sha256:<64-lowercase-hex>",
...
func (store RuntimeStore) runtimeHandoffStatusExit() string {
	return fmt.Sprintf("run `hgtran-ai sdd-attempt status --cwd %s --change %q` to read the active attempt and its current execution worktree", pathquote.Quote(store.Workspace), store.Change)
}
...
// directory: <common>/hgtran-ai/sdd-runtime/v1/<change>/records/<sha256>.json.
...
func (err *RuntimeRecordSchemaUnsupportedError) Error() string {
	return fmt.Sprintf("SDD runtime revision %s declares \"schema\" %s, newer than this binary supports (%s); run `hgtran-ai update` to install a build that reads it, then rerun the same `hgtran-ai sdd-attempt` command", err.Revision, err.Schema, runtimeRecordSchema)
}
