...
const changeInstanceMarkerFile = ".hgtran-ai-instance"
...
func sddConsentGrantRequestID(change, instance, expectedRevision string, roots []string) string {
	hash := sha256.New()
	for _, part := range append([]string{"hgtran-ai.sdd-consent-grant-request/v1", change, instance, expectedRevision}, roots...) {
...
func newEditAuthorityConsent(change, workspaceRoot string, missingRoots []string, instance, expectedRevision string) SDDIntegrationConsentResult {
	statusInvocation := fmt.Sprintf("hgtran-ai sdd-status %s --cwd %s", change, pathquote.Quote(workspaceRoot))
