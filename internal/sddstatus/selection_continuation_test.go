...
// `hgtran-ai sdd-status --cwd <repo> --change <name>`, but ParseCommandArgs
...
//
// The guard: every runnable `hgtran-ai sdd-status ...` continuation emitted in
...
var sddStatusContinuationRe = regexp.MustCompile("`hgtran-ai sdd-status ([^`]+)`")
