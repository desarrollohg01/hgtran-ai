...
func TestRuntimeLedgerRefusesNewerRecordSchemaByName(t *testing.T) {
	...
	rewriteRuntimeHeadRecord(t, store, func(payload []byte) []byte {
		return bytes.Replace(payload, []byte(runtimeRecordSchema), []byte("hgtran-ai.sdd-runtime-record/v2"), 1)
	})
	_, err := store.Status()
	if err == nil || !strings.Contains(err.Error(), `"schema"`) || !strings.Contains(err.Error(), "hgtran-ai update") {
		t.Fatalf("newer schema status error = %v, want it to name the schema field and `hgtran-ai update`", err)
	}
}
...
func TestOpenRuntimeStoreOutsideGitRepositoryNamesGitInit(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "git init") || !strings.Contains(err.Error(), "hgtran-ai sdd-attempt") || strings.Contains(err.Error(), "rev-parse") {
