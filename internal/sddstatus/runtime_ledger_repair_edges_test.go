	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if record.Operation == runtimeOperationRescope {
				...
				record.RequestDigest = runtimeValueHash("hgtran-ai.sdd-runtime-rescope-request/v1", request)
			}
...
func TestRuntimeConsecutiveRescopeRepairContinuationQuotesDynamicArguments(t *testing.T) {
	...
	// arguments without launching Hgtran AI. A broken quote would create marker.
	script := "set -- " + strings.TrimPrefix(command, "hgtran-ai ") + "\nfor argument in \"$@\"; do printf '%s\\000' \"$argument\"; done"
...
func TestRuntimeConsecutiveRescopeRepairRequiredErrorPrintsContinuationOnNewLine(t *testing.T) {
	...
	const continuation = "hgtran-ai sdd-attempt repair --cwd /tmp/repo --change repair-2839"
