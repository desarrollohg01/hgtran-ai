...
func TestSDDStatusV2CleanBreak(t *testing.T) {
	t.Run("v2 is the sole default and v1 is refused read-only", func(t *testing.T) {
		...
		if defaultArgs.Contract != "hgtran-ai.sdd-status/v2" {
			t.Fatalf("default contract = %q, want hgtran-ai.sdd-status/v2", defaultArgs.Contract)
		}

		v2Args, err := ParseCommandArgs([]string{"thin", "--contract", "hgtran-ai.sdd-status/v2"})
		if err != nil {
			t.Fatalf("ParseCommandArgs(v2) error = %v", err)
		}
		if v2Args.Contract != "hgtran-ai.sdd-status/v2" {
			t.Fatalf("v2 contract = %q, want hgtran-ai.sdd-status/v2", v2Args.Contract)
		}

		_, err = ParseCommandArgs([]string{"thin", "--contract", "hgtran-ai.sdd-status/v1"})
		if err == nil || err.Error() != "unsupported sdd-status contract \"hgtran-ai.sdd-status/v1\". "+freshV2RerunInstruction {
	...
	t.Run("shipped assets and goldens no longer pin v1", func(t *testing.T) {
		...
		if golden := mustReadStatusGolden(t, "sdd-claude-cmd-hgtran-sdd-status.golden"); strings.Contains(golden, "sdd-status/v1") || strings.Contains(golden, "Native status v1") {
		...
		t.Run(string(store), func(t *testing.T) {
			...
			if store == ArtifactStoreOpenSpec {
				...
				restore := stubEngramExport(t, []engramObservation{
					...
					{Title: "sdd/" + change + "/verify-report", Content: testVerifyEnvelope("pass", 0, 0, "1/1", "1/1", 0, 0), Project: "hgtran-ai", Scope: "project"},
				})
				t.Cleanup(restore)
			}
...
func retiredV1StatusTestPins(content string) []string {
	...
	if strings.Contains(content, "hgtran-ai.sdd-status/v1") {
