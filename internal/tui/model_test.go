...
func TestStepProgressMsgAddsNestedPackageProgress(t *testing.T) {
	const packageStep = "agent:pi:pi install npm:hgtran-pi"
...
func TestInstallPipelineProgressDeliveryDoesNotBlockWithoutReceiver(t *testing.T) {
	...
	m.ExecuteFn = func(
		...
		progress(pipeline.ProgressEvent{StepID: "agent:pi:pi install npm:hgtran-pi", Status: pipeline.StepStatusRunning})
		return pipeline.ExecutionResult{}
	}
...
func TestModelConfigOpenCodePrePopulatesAssignments(t *testing.T) {
	...
	preExisting := map[string]model.ModelAssignment{
		"hgtran-orchestrator": {ProviderID: "anthropic", ModelID: "claude-sonnet-4-20250514"},
	...
	got := state.Selection.ModelAssignments["hgtran-orchestrator"]
	want := preExisting["hgtran-orchestrator"]
	if got != want {
		t.Errorf("hgtran-orchestrator assignment = %+v, want %+v", got, want)
	}
...
func TestModelConfigOpenCodeDoesNotOverwriteExistingSessionAssignments(t *testing.T) {
	...
	m.Selection.ModelAssignments = map[string]model.ModelAssignment{
		"hgtran-orchestrator": sessionAssignment,
	}
...
func TestComponentsForPreset_PersonaMatrix(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "full-hgtran + hgtran includes persona and safe agent visuals",
		...
		{
			name:             "full-hgtran + custom excludes persona but keeps safe agent visuals",
			preset:           model.PresetFullGentleman,
		...
		{
			name:        "minimal + hgtran includes persona",
		...
		{
			name:    "custom preset returns nil regardless of persona (hgtran)",
...
func TestPersonaScreenRecomputesComponentsWhenPresetAlreadySet(t *testing.T) {
	// Start with a model that has already picked full-hgtran preset and
	// hgtran persona (the default), then go back to Persona screen and pick custom.
	m := NewModel(system.DetectionResult{}, "dev")
	m.Screen = ScreenPersona
	m.Selection.Preset = model.PresetFullGentleman
...
func TestOpenCodePluginUninstallDetectsInstalledFromTUIJSON(t *testing.T) {
  ...
  "plugin": [
    ...
    "unrelated-plugin",
    "/home/me/.config/opencode/tui-plugins/hgtran-logo.tsx"
  ]
}`
