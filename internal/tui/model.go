...
import (
	...
	"github.com/hgtran-programming/hgtran-ai/v2/internal/agentbuilder"
...
func (m Model) confirmSelection() (tea.Model, tea.Cmd) {
	...
	case ScreenProfiles:
		...
		case m.Cursor < profileCount:
			...
			// The ModelPicker shows hgtran-orchestrator as the base row, so we need
...
func openCodePluginUninstallInstalledFromTUI(home string) []model.OpenCodeCommunityPluginID {
	...
	gentleLogoSuffixes := []string{
		filepath.Join("tui-plugins", "hgtran-logo.tsx"),
		"tui-plugins/hgtran-logo.tsx",
	}
...
// provided by hgtran-pi itself, so the pi-only flow (which skips the SDD
