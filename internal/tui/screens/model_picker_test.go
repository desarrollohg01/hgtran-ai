...
func TestRenderModelPickerScrollsToReviewAgents(t *testing.T) {
	...
	if strings.Contains(output, "hgtran-orchestrator") {
...
func TestModelPickerRows_OrchestratorIsFirst(t *testing.T) {
	rows := ModelPickerRows()
	if rows[0] != "hgtran-orchestrator" {
		t.Fatalf("ModelPickerRows()[0] = %q, want %q", rows[0], "hgtran-orchestrator")
	}
}
...
func TestHandleModelNav_OrchestratorRowAssignsOnlyOrchestrator(t *testing.T) {
	state := makeTestState(0) // row 0 = hgtran-orchestrator
	...
	// "hgtran-orchestrator" key must be set
...
func TestHandleModelNav_SetAllPhasesRow_SetsOnlySubAgents(t *testing.T) {
	...
	// hgtran-orchestrator must NOT be touched by "Set all phases"
	if _, exists := updated[SDDOrchestratorPhase]; exists {
		t.Errorf("hgtran-orchestrator should NOT be assigned by 'Set all phases'; assignments: %v", updated)
	}
}
...
func TestHandleModelNav_SubAgentRow_AssignsCorrectPhase(t *testing.T) {
	...
	for i, expectedPhase := range phases {
		t.Run(expectedPhase, func(t *testing.T) {
			...
			if _, exists := updated[SDDOrchestratorPhase]; exists {
				t.Errorf("hgtran-orchestrator should not be assigned; assignments: %v", updated)
			}
		})
	}
}
...
func TestSDDOrchestratorPhaseConstant(t *testing.T) {
	if SDDOrchestratorPhase != "hgtran-orchestrator" {
		t.Fatalf("SDDOrchestratorPhase = %q, want %q", SDDOrchestratorPhase, "hgtran-orchestrator")
	}
}
...
func TestHandleEffortNav_SetAllPhasesUpdatesAllPhasesModelAndAllSubAgents(t *testing.T) {
	...
	// hgtran-orchestrator must NOT be touched by "Set all phases".
	if _, exists := updated[SDDOrchestratorPhase]; exists {
		t.Errorf("hgtran-orchestrator should NOT be assigned by Set all phases effort")
	}
}
