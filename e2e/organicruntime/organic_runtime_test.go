...
func TestCodexProviderAdapterUsesPinnedLocalRuntime(t *testing.T) {
	...
	statusPayload := harness.hgtran(
		"review", "status", "--cwd", harness.repo.worktree, "--contract", "hgtran-ai.review-integration/v2",
	...
	stdout, stderr, err := harness.gentleAllowFailure(
		"review", "start", "--cwd", harness.repo.worktree, "--contract", "hgtran-ai.review-integration/v2",
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Method == http.MethodGet && request.URL.Path == "/v1/models" {
			...
			_, _ = fmt.Fprint(writer, `{"object":"list","data":[{"id":"gpt-5.6-terra","object":"model","created":0,"owned_by":"hgtran-ai-loopback"}]}`)
			return
		}
...
`), 0o755); err != nil {
	...
	continuationPayload := harness.hgtran(continuationArguments...)
...
func TestOpenCodeRuntimeIsPinnedForTheLiveProviderTransport(t *testing.T) {
	...
	config, err := json.Marshal(map[string]any{
		...
		"provider": map[string]any{"loopback": map[string]any{
			"npm": "@ai-sdk/openai-compatible", "name": "Hgtran AI loopback",
...
func TestOpenCodeRuntimeRunsFourBoundReviewersConcurrently(t *testing.T) {
	...
	stdout, stderr, err := harness.gentleAllowFailure(
		...
		"--contract", "hgtran-ai.review-integration/v2", "--target", start.argument("target"), "--projection", start.argument("projection"),
	...
	config, err := json.Marshal(map[string]any{
		...
		"provider": map[string]any{"loopback": map[string]any{
			"npm": "@ai-sdk/openai-compatible", "name": "Hgtran AI 4R loopback",
			"options": map[string]any{"baseURL": server.URL + "/v1", "apiKey": "loopback-key"},
			"models":  map[string]any{"loopback": map[string]any{"name": "Loopback", "limit": map[string]int{"context": 32000, "output": 2048}}},
		}},
		"agent": reviewers,
...
func organicProviderStart(t *testing.T, harness *organicHarness, lineage, agent string) organicStartResult {
	...
	stdout, stderr, err := harness.gentleAllowFailure("review", "start", "--cwd", harness.repo.worktree,
		"--contract", "hgtran-ai.review-integration/v2", "--target", transition.argument("target"), "--projection", transition.argument("projection"),
...
func organicProviderStatus(t *testing.T, harness *organicHarness, lineage, agent string) organicProviderStatusResult {
	t.Helper()
	payload := harness.hgtran("review", "status", "--cwd", harness.repo.worktree, "--contract", "hgtran-ai.review-integration/v2", "--agent", agent, "--lineage", lineage, "--next-transition", "--projection", "workspace")
...
func TestOrganicConfiguredAgentReceivesRoutingGuidanceCursor(t *testing.T) {
	...
	// never has. Simulate Cursor as already installed so hgtran-ai does not
	...
	const path = ".cursor/rules/hgtran-ai.mdc"
...
func TestOrganicBoundedCorrectionAllowsExactlyOne(t *testing.T) {
	...
	payload := harness.hgtran(planArguments...)
...
func TestOrganicRuntimeCurrentReviewHardening(t *testing.T) {
	...
	t.Run("issue-1666-and-1807-policy-preflight", func(t *testing.T) {
		for _, test := range []struct {
			...
			t.Run(test.name, func(t *testing.T) {
				...
				if _, statErr := os.Stat(filepath.Join(harness.commonDir(), "hgtran-ai", "defect-reports")); !os.IsNotExist(statErr) {
	...
	t.Run("issue-1813-store-quarantine", func(t *testing.T) {
		...
		planPayload := harness.hgtran(planArguments...)
		var planned struct {
			Operation string `json:"operation"`
			State     string `json:"state"`
		}
		...
		statePath := filepath.Join(harness.commonDir(), "hgtran-ai", "review-transactions", "v2", lineage, "review-state.json")
		...
		inventoryPayload := harness.hgtran("review", "status", "--cwd", harness.repo.worktree)
		...
		explicitPayload := harness.hgtran(
			"review", "status", "--cwd", harness.repo.worktree,
			"--contract", "hgtran-ai.review-integration/v2", "--agent", "opencode", "--lineage", lineage, "--next-transition",
		)
	...
	t.Run("selector-free-status-is-silent", func(t *testing.T) {
		...
		stdout, stderr, err := harness.gentleAllowFailure(
			"review", "status", "--cwd", harness.repo.worktree,
			"--contract", "hgtran-ai.review-integration/v2", "--agent", "opencode", "--next-transition",
		)
		if err != nil {
	...
	t.Run("occupied-reviewer-slot-has-no-report", func(t *testing.T) {
		...
		if !strings.Contains(conflictStderr, "reviewer_result_slot_occupied") ||
			!strings.Contains(conflictStderr, "hgtran-ai review status --cwd <repo> --contract hgtran-ai.review-integration/v2 --next-transition") ||
...
func TestOrganicTerminalAuthoritySurvivesWithdrawalAndReplaysWithoutEffect(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(harness.commonDir(), "hgtran-ai", "review-transactions", "v2", lineage)); !os.IsNotExist(err) {
...
func (harness *organicHarness) hgtran(arguments ...string) []byte {
	harness.t.Helper()
...
func organicApprovedAcknowledgementStatus(t *testing.T, harness *organicHarness, lineage string, selectors ...string) *organicProviderExecute {
	...
	payload := harness.hgtran(arguments...)
	var status organicProviderStatusResult
	if err := json.Unmarshal(payload, &status); err != nil {
...
func (harness *organicHarness) assertReviewAcknowledgedAndBurned(lineage string, finalized organicFinalizeResult) {
	...
	authority := filepath.Join(harness.commonDir(), "hgtran-ai", "review-transactions", "v2", lineage)
...
func (harness *organicHarness) captureReviewerResult(lineage string, started organicStartResult, order int, result organicReviewerResult) (string, string, error) {
	...
	if err := json.Unmarshal(harness.hgtran(append(binding, "--preflight")...), &preflight); err != nil {
...
func (harness *organicHarness) disableReview() organicModeResult {
	harness.t.Helper()
	payload := harness.hgtran("review", "mode", "disable", "--cwd", harness.repo.worktree, "--scope", "clone", "--json")
	var mode organicModeResult
...
func (harness *organicHarness) enableReviewGlobally() organicModeResult {
	harness.t.Helper()
	payload := harness.hgtran("review", "mode", "enable", "--cwd", harness.repo.worktree, "--scope", "global", "--json")
	var mode organicModeResult
	if err := json.Unmarshal(payload, &mode); err != nil {
		harness.t.Fatalf("decode review mode: %v\n%s", err, payload)
	}
...
func (harness *organicHarness) enableReview() organicModeResult {
	harness.t.Helper()
	payload := harness.hgtran("review", "mode", "enable", "--cwd", harness.repo.worktree, "--scope", "clone", "--json")
	var mode organicModeResult
	if err := json.Unmarshal(payload, &mode); err != nil {
		harness.t.Fatalf("decode review mode: %v\n%s", err, payload)
	}
...
func (harness *organicHarness) sddStatus(change string) organicSDDStatus {
	harness.t.Helper()
	payload := harness.hgtran("sdd-status", change, "--cwd", harness.repo.worktree, "--json")
...
func (harness *organicHarness) runNamedReviewStart(tokens []string, extra ...string) organicStartResult {
	...
	payload := harness.hgtran(append(append([]string{}, tokens...), extra...)...)
	var started organicStartResult
...
func (harness *organicHarness) reviewModeGenerations() []string {
	harness.t.Helper()
	root := filepath.Join(harness.commonDir(), "hgtran-ai", "review-mode", "rar-authority", "v1", "rdd-mode")
...
func harnessCorrectionStatus(t *testing.T, harness *organicHarness, lineage string) organicCorrectionStatus {
	t.Helper()
	payload := harness.hgtran(
		"review", "status", "--cwd", harness.repo.worktree, "--lineage", lineage,
		"--contract", "hgtran-ai.review-integration/v2", "--next-transition",
	)
...
func TestRealAgentInstalledSDDApplyExecutorDoesNotDelegate(t *testing.T) {
	...
	defer cancel()
	command := organicCommandContext(ctx, "opencode", "run", "--pure",
		"--format", "json", "--agent", "hgtran-orchestrator", "--model", "fixture/fixture",
