...
// consentShellEnvironment prepends a directory holding a `hgtran-ai` shim for
...
func runConsentInvocation(t *testing.T, environment []string, dir, invocation string) {
	t.Helper()
	if !strings.HasPrefix(invocation, "hgtran-ai ") {
		t.Fatalf("invocation is not a hgtran-ai command: %q", invocation)
	}
...
func TestSDDEditAuthorityConsentGrantLoop(t *testing.T) {
	...
	if blocked.Consent.Schema != "hgtran-ai.sdd-integration.consent/v1" || blocked.Consent.Change != change ||
...
func TestSDDSameParentRepositoryConsentGrantLoop(t *testing.T) {
	...
	if blocked.Consent == nil || blocked.Consent.Schema != "hgtran-ai.sdd-integration.consent/v1" ||
...
func TestSDDSingleRepoStatusStaysByteIdenticalWithZeroConsentFootprint(t *testing.T) {
	...
	if _, err := os.Lstat(filepath.Join(planning, "openspec", "changes", change, ".hgtran-ai-instance")); !os.IsNotExist(err) {
