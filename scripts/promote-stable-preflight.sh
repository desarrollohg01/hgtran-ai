...
stable_release=$(gh api graphql -f 'query=query($tag: String!) { repository(owner: "Gentleman-Programming", name: "hgtran-ai") { release(tagName: $tag) { databaseId } } }' -f "tag=$stable_tag" --jq '.data.repository.release.databaseId // empty')
...
if [[ -z "$stable_ref" ]]; then
  ...
  if [[ -z "$stable_release" ]]; then
    ...
    if jq -e '.draft == true and (.assets | length) == 0' <<<"$release" >/dev/null; then
      ...
      diff -u <(printf '%s\n' "hgtran-ai_${version}_darwin_amd64.tar.gz" "hgtran-ai_${version}_darwin_arm64.tar.gz" "hgtran-ai_${version}_linux_amd64.tar.gz" "hgtran-ai_${version}_linux_arm64.tar.gz" "hgtran-ai-review-provider-contract-${provider_contract_semver}.tar.gz" checksums.txt checksums.txt.minisig | LC_ALL=C sort) <(jq -r '.assets[].name' <<<"$release" | LC_ALL=C sort); then
