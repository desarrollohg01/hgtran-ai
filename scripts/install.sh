...
print_homebrew_failure_help() {
    ...
    if [[ "$lower" == *"untrusted tap"* || "$lower" == *"tap trust is required"* || "$lower" == *"homebrew_require_tap_trust"* ]]; then
        ...
        echo "Trust only the Hgtran AI formula, then retry:" >&2
...
show_help() {
    cat <<EOF
${BOLD}hgtran-ai installer${NC}

Usage: install.sh [OPTIONS]

Options:
  --method METHOD   Force install method: brew, go, binary (default: auto-detect)
  --channel CHANNEL Hgtran AI channel: stable (default), beta, or nightly (env: HGTRAN_AI_CHANNEL)
...
# Examples:
#   hgtran-ai_1.0.0_darwin_arm64.tar.gz
#   hgtran-ai_1.0.0_linux_amd64.tar.gz
# ============================================================================
...
detect_install_method() {
    if [ "${CHANNEL}" = "beta" ]; then
        if [ -n "${FORCE_METHOD:-}" ] && [ "${FORCE_METHOD}" != "go" ]; then
            fatal "--channel beta installs Hgtran AI from main and only supports --method go"
        fi
        INSTALL_METHOD="go"
...
print_banner() {
    ...
    echo -e "${NC}"
    echo -e "  ${DIM}hgtran-ai — Ecosystem, Frameworks, Workflows${NC}"
    echo ""
}
...
print_next_steps() {
    ...
    if [ "${CHANNEL}" = "beta" ]; then
        echo -e "  ${CYAN}1.${NC} Run ${BOLD}HGTRAN_AI_CHANNEL=beta ${BINARY_NAME} install${NC} to keep using the beta channel"
    else
...
main() {
    ...
    INSTALL_DIR=""
    INSECURE="false"
    CHANNEL="${HGTRAN_AI_CHANNEL:-stable}"
