...
func (b *battery) runCommandLineEnv(source, dir string, env []string, command string) (map[string]any, string, int) {
	...
	if err != nil || len(words) < 2 || words[0] != "hgtran-ai" {
...
func (b *battery) hostNegotiatedMediumStart(lane, repo, agent string, env []string) bool {
	...
	if getString(consent, "schema") != "hgtran-ai.review-integration.consent/v3" || getString(consent, "action") != "consent_required" {
...
// the installed hgtran-pi relay implementation instead.
...
func (b *battery) runHostLanes() {
	if !b.withHost {
		...
		b.skip(hostPiLane, "typed SKIP: separate hgtran-pi dev-binary evidence contract", "default battery does not fake/copy the relay; --with-host is independent live-host proof")
