...
// INSTALLED hgtran-pi review-host-relay implementation (byte-identical lib
...
// piRelayContractEnv is the exact handshake hgtran-ai requires before the pi
// runtime identity becomes eligible for immutable review transport.
var piRelayContractEnv = []string{"GENTLE_PI_REVIEW_RELAY_CONTRACT=hgtran-pi.review-relay/v1"}

// piPackageDir returns the installed hgtran-pi package root.
func piPackageDir() string {
	...
	return filepath.Join(home, ".pi", "agent", "npm", "node_modules", "hgtran-pi")
}
...
// stays out of this battery is driving hgtran-pi's ORCHESTRATOR review flow
...
// hgtran-pi's launcher meets the Go facade, with real pi processes at every
...
func (b *battery) runHostPiLane() {
	...
	if _, err := exec.LookPath("node"); err != nil {
		b.skip(hostPiLane, "pi host lane", "node is unavailable for the hgtran-pi relay driver")
		return
	}
	packageDir := piPackageDir()
	if packageDir == "" || !piRelayClosurePresent(packageDir) {
		b.skip(hostPiLane, "pi host lane", "installed hgtran-pi package with lib/review-host-relay.ts not found under "+packageDir)
		return
	}
	...
	if err != nil {
		b.fail(hostPiLane, "hgtran-pi relay driver setup", err.Error())
		return
	}
...
// preparePiRelayDriver copies the installed hgtran-pi lib/ and scripts/
...
func (b *battery) preparePiRelayDriver(packageDir string) (string, error) {
	relayDir := filepath.Join(b.workRoot, "hgtran-pi-relay")
	for _, sub := range []string{"lib", "scripts"} {
		...
		if err != nil {
			return "", fmt.Errorf("read installed hgtran-pi %s/: %w", sub, err)
		}
...
// input through the installed hgtran-pi relay: materialize -> real locked-down
// pi subprocess -> provider-owned submission, all owned by hgtran-pi's code.
func (b *battery) runPiRelaySlot(lane, repo string, input map[string]any) bool {
	const check = "relay capture admitted"
	if b.piRelayDir == "" {
		b.fail(lane, check, "hgtran-pi relay driver is not prepared")
		return false
	}
	...
	b.noteHostCost(lane, "1 real pi print-mode reviewer run (hgtran-pi host relay)")
	...
	if err != nil {
		...
		b.fail(lane, check, fmt.Sprintf("hgtran-pi relay driver failed: %v %s", err, firstLine(detail)))
		return false
	}
	...
	if getString(manifest, "schema") != "hgtran-ai.review-result-artifact/v2" {
		b.fail(lane, check, "relay submission did not round-trip a native result artifact")
		return false
	}
	b.pass(lane, check, fmt.Sprintf("installed hgtran-pi relay ran a real pi process (%d prompt bytes -> %d result bytes) and the capture was admitted", result.PromptBytes, result.ResultBytes))
	return true
}
