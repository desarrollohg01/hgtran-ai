...
//
// It drives one real hgtran-ai binary (--binary) end to end across the
...
func run() int {
	binary := flag.String("binary", "", "path to the hgtran-ai binary under test (required)")
	...
	workRoot, err := os.MkdirTemp("", "hgtran-ai-crosslane-")
	if err != nil {
		fmt.Fprintf(os.Stderr, "crosslane: %v\n", err)
		return 2
	}
