...
// `opencode run` in a sandboxed HOME with the real hgtran-ai transport plugin
...
// through a live `hgtran-ai review opencode-transport` child, and a real
...
func (b *battery) runHostOpenCodeLane() {
	...
	// The whole lane, hgtran-ai included, runs under the sandbox HOME: the
...
// realOpenCodeReviewAgents extracts the hgtran-ai-synced review-* subagent
...
func realOpenCodeReviewAgents(realHome string) (map[string]any, error) {
	...
	if len(review) == 0 {
		return nil, fmt.Errorf("no review-* agents; run hgtran-ai sync for OpenCode first")
	}
	return review, nil
}
...
// openCodeSandboxGentleEnvironment is the override set for hgtran-ai
...
// whose first entry resolves `hgtran-ai` to the binary under test.
...
// hostShimPath builds a PATH whose first entry resolves `hgtran-ai` to the
...
func (b *battery) hostShimPath() string {
	...
	if err := os.MkdirAll(shimDir, 0o755); err == nil {
		shim := "#!/bin/sh\nexec \"" + b.binary + "\" \"$@\"\n"
		_ = os.WriteFile(filepath.Join(shimDir, "hgtran-ai"), []byte(shim), 0o755)
	}
