...
// bytes, the hook emulator, and a PATH shim so the plugin's spawn("hgtran-ai")
...
func (b *battery) prepareHookHarness(repo string) (string, error) {
	...
	shim := "#!/bin/sh\nexec \"" + b.binary + "\" \"$@\"\n"
	if err := os.WriteFile(filepath.Join(dir, "bin", "hgtran-ai"), []byte(shim), 0o755); err != nil {
		return "", err
	}
	_ = repo
	return dir, nil
...
func (b *battery) checkValidatorInspectionRecipe(repo, childPrompt string) {
	...
	if !strings.Contains(childPrompt, "hgtran-ai review inspect-candidate") {
...
func (b *battery) runHookCase(harnessDir string, c harnessCase) (harnessResult, error) {
	...
	// The harness spawns hgtran-ai itself, so it has to inherit the battery's
