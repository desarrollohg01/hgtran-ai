...
const (
	...
	reviewContract = "hgtran-ai.review-integration/v2"
)
...
func (b *battery) startCommittedMedium(lane, repo, agent, baseTree string) bool {
	...
	if code != 0 || getString(consent, "schema") != "hgtran-ai.review-integration.consent/v3" || getString(consent, "action") != "consent_required" {
...
func admittedCapture(doc map[string]any) bool {
	...
	case "hgtran-ai.review-result-artifact/v2":
		return getString(doc, "admission_decision") == "completed"
	case "hgtran-ai.review-last-event-closure/v1":
