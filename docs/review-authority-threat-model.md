...
After committing a staged candidate, use `hgtran-ai review start --projection staged --base-ref <ref>` (and `--committed-only` when tracked workspace changes exist) to record immutable base-to-HEAD review provenance. It accepts no intended-untracked paths, remains domain-separated in the v2 identity, and can reuse only the matching staged authority; an otherwise identical workspace authority remains separate.
...
Run `hgtran-ai review schema reviewer`, `hgtran-ai review schema refuter`, or `hgtran-ai review schema validator` to print a versioned JSON Schema. Each admitted capture is bound to its immutable candidate, revision, and selected role. Unknown fields and semantic violations are rejected before authority changes.
