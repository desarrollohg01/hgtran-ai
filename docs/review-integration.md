...
`hgtran-ai.review-integration/v2` coordinates one immutable review transaction at a time. Go owns the candidate snapshot, review admission, correction boundary, terminal burn, and all provider-facing bindings. Claude Code, OpenCode, Codex, and Pi transport provider-issued work; no runtime adapter decides review or delivery.

## RDD starts off

RDD is opt-in. Until a user enables it with `hgtran-ai review mode enable --scope global`, review does not govern the candidate and delivery follows ordinary repository policy. Disabling returns to that state. Enabling revalidates the current candidate; it never resumes stale authority.
...
The acknowledgement transition is an execution detail of `hgtran-ai.review-integration/v2`; it does not authorize delivery. Escalated and other non-approved terminal paths retain their existing response and STATUS behavior and do not gain an acknowledgement transition. After a malformed, incomplete, or unavailable capture, retain the exact lineage, revision, and target binding, query bound STATUS once, and follow only the reoffered capture route.
...
Medium and high-risk START may return the typed `hgtran-ai.review-integration.consent/v3` envelope. Relay the complete choice envelope losslessly, preserve machine tokens and invocations exactly, and run only the invocation selected by the human. Global RDD mode permits review; it never grants per-candidate consent. A decline is not the kill switch.

## Delivery remains human-owned

`hgtran-ai review validate` and named gates (`post-apply`, `pre-commit`, `pre-push`, `pre-pr`, and `release`) are compatibility/informational commands. They never discover authority or decide delivery:
...
| `manual_intervention_required` | Terminal — the authority state is outside the negotiated lifecycle. Ask a maintainer to inspect it, or run `hgtran-ai review mode disable --scope clone --cwd <repo>`. |
| `missing_authority_binding` | Terminal — a current target had no authority binding. File a bounded defect with the lineage, or run `hgtran-ai review mode disable --scope clone --cwd <repo>`. |
| `native_stop_required` | Terminal — the lineage is escalated but has no native continuation. Ask a maintainer to inspect it, or run `hgtran-ai review mode disable --scope clone --cwd <repo>`. |
| `recovery_scope_unchanged` | Change B so its target identity differs, then retry the exact returned `hgtran-ai review recover` invocation. |
| `staged_workspace_overlay_recovery_unavailable` | Terminal — pass `--lineage <id>` to recover an existing lineage, or drop `--workspace-overlay` and start a fresh target; otherwise run `hgtran-ai review mode disable --scope clone --cwd <repo>`. |
| `rdd_disabled` | Run the exact source-scoped `hgtran-ai review mode enable` command rendered by STATUS, then re-run its exact repository-bound STATUS command. |
