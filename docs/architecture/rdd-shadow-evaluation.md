...
2. To opt in for local investigation, set `HGTRAN_AI_RDD_SHADOW=1` before running a gate (`post-apply`, `pre-commit`, `pre-push`, `pre-pr`, `release`) or `hgtran-ai review start`/`status`.
