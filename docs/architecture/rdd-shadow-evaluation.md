...
2. To opt in for local investigation, set `GENTLE_AI_RDD_SHADOW=1` before running a review-context hook (`post-apply`, `pre-commit`, `pre-push`, `pre-pr`, `release`) or `hgtran-ai review start`/`status`.
