...
1. Leave `HGTRAN_AI_RDD_SHADOW` unset. This is the default and the only supported setting for normal use — shadow evaluation is off, and zero shadow code runs.
2. To opt in for local investigation, set `HGTRAN_AI_RDD_SHADOW=1` before running a review-context hook (`post-apply`, `pre-commit`, `pre-push`, `pre-pr`, `release`) or `hgtran-ai review start`/`status`.
3. Watch **stderr** (never stdout) for one `hgtran-ai.rdd-shadow/v1` line per observed call.
...
```
hgtran-ai.rdd-shadow/v1 gate=<GateKind> live_result=<GateResult> has_relation=<bool> shadow_relation=<ShadowRelation> no_live_counterpart=<bool> authority_health=<healthy|repairable|blocked> err=<quoted string, empty when none>
```
...
2. Open an issue: <https://github.com/desarrollohg01/hgtran-ai/issues/new/choose>. Include the line, the gate or command that produced it, and whether it repeats.
