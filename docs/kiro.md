...
This document explains how hgtran-ai integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

hgtran-ai supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, hgtran-ai installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/hgtran-ai.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running hgtran-ai.
...
hgtran-ai uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `hgtran-ai install`.
...
This follows the same SDD architecture used in hgtran-ai: orchestrator coordinates, phase agents execute, Engram persists artifacts across phases.
...
Kiro has a built-in spec workflow that hgtran-ai leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
The steering file written by hgtran-ai uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\hgtran-ai.md` |
...
Kiro uses a **split-root layout** — hgtran-ai managed files and IDE settings live in different directories:

- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/hgtran-ai.md` — orchestrator persona
...
If hgtran-ai skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
