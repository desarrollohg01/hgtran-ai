...
```powershell
gh issue view <N> --repo desarrollohg01/hgtran-ai --json number,url,title,state,labels,author,createdAt,updatedAt,closedAt
```
...
```powershell
gh pr view <N> --repo desarrollohg01/hgtran-ai --json number,url,title,state,isDraft,author,createdAt,updatedAt,closedAt,mergedAt,headRefName,baseRefName
```
...
```powershell
gh api -H "Accept: application/vnd.github+json" repos/desarrollohg01/hgtran-ai/issues/<N>/timeline --paginate --jq '.[] | select(.event == "cross-referenced") | {number: .source.issue.number, state: .source.issue.state, url: .source.issue.html_url}'
```
...
```powershell
gh search prs --repo desarrollohg01/hgtran-ai --state open --match title,body --json number,title,url,updatedAt -- "#<N>"
```
...
```powershell
gh issue view 1770 --repo desarrollohg01/hgtran-ai --json number,title,state,body,comments,labels,author,createdAt,updatedAt,url
gh issue view 1806 --repo desarrollohg01/hgtran-ai --json number,title,state,body,comments,labels,author,createdAt,updatedAt,url
gh issue view 1834 --repo desarrollohg01/hgtran-ai --json number,title,state,body,comments,labels,author,createdAt,updatedAt,url
gh issue view 1839 --repo desarrollohg01/hgtran-ai --json number,title,state,body,comments,labels,author,createdAt,updatedAt,url
gh pr view 1073 --repo desarrollohg01/hgtran-ai --json number,title,state,body,files,commits,closingIssuesReferences,url
gh pr view 973 --repo desarrollohg01/hgtran-ai --json number,title,state,body,files,commits,closingIssuesReferences,url
git status --short
git branch --show-current
git rev-parse HEAD
$repoRoot = git rev-parse --show-toplevel
