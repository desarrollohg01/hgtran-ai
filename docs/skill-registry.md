...
Use `hgtran-ai skill-registry refresh` after you add, remove, rename, or move skills. Normal installs wire this refresh into startup hooks where the agent supports them, including Codex, Claude Code, OpenCode, and Pi through `gentle-pi`.
...
Audits each indexed SKILL.md against docs/skill-style-guide.md
   ...
   │
   ▼
Run hgtran-ai skill-registry refresh again
```
...
```bash
hgtran-ai skill-registry list          # name<TAB>scope<TAB>path
hgtran-ai skill-registry list --json   # machine-readable, includes descriptions
```
