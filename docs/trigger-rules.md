...
| `hgtran-ai review mode enable --scope global --cwd <repo>` | Opt in. Enables receipt-driven development globally for future candidates. This is the only command that turns it on. |
...
| `hgtran-ai review mode enable --scope clone --cwd <repo>` | Clear this clone's off-only override. Does not turn review on by itself. |
...
| Command | Effect |
|---|---|
| `hgtran-ai review store-reset --cwd <repo>` | Report, per category, what a reset would remove and what it would preserve. Removes nothing. |
| `hgtran-ai review store-reset --cwd <repo> --confirm` | Remove this clone's review lineage state. Irreversible. |
| `hgtran-ai review store-reset --cwd <repo> --confirm --include-in-flight` | Also remove reviews that have not reached a terminal state. |
| `hgtran-ai review store-reset --cwd <repo> --confirm --include-adapter-reviews` | Also remove the adapter-written `reviews/` graph store, which the lease and the in-flight refusal do not cover. |
| `hgtran-ai review store-reset --cwd <repo> --json` | The same report, machine-readable. |
...
It **withholds** `reviews/`, the review graph store the hgtran-pi adapter
...
- OpenCode and Kilocode receive it inside
  `agent.hgtran-orchestrator.prompt` in their adapter-owned `opencode.json`.
