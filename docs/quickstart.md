...
- `git` available.
- If Node.js is missing, `hgtran-ai install` prints this install hint: NodeSource LTS setup + `apt-get install -y nodejs` (npm comes bundled).
...
- `sudo` access for package installs.
- `git` available.
- If Node.js is missing, `hgtran-ai install` prints this install hint: `pacman -S --noconfirm nodejs npm`.
...
- `sudo` access for package installs.
- `git` available.
- If Node.js is missing, `hgtran-ai install` prints this install hint: NodeSource LTS setup + `dnf install -y nodejs` (npm comes bundled).
...
- Go 1.25.10+, because Windows installs and upgrades through `go install`.
  ...
  unsigned is ever fetched. With Go on `PATH`, `hgtran-ai upgrade` updates
  itself automatically by running `go install …/cmd/hgtran-ai@vX.Y.Z` pinned to
...
```bash
go run ./cmd/hgtran-ai install --dry-run
```
...
The agents you select during install become the default scope for future `hgtran-ai sync` runs. Hgtran AI records that selection in `~/.hgtran-ai/state.json` and does not automatically sync every agent config directory that exists on your machine. To check what will be updated after an upgrade, run:
...
Hgtran AI pins versions and disables postinstall scripts on every npm install it generates. When you install the `permissions` component, a sensitive-paths deny list is applied to Claude Code and OpenCode blocking access to `~/.ssh/*`, `**/*.pem`, `**/*.key`, `**/.env*`, `~/.aws/credentials`, and other credential paths. See [Components](../docs/components.md) for the full list.
