...
|---|---|
| `cmd/hgtran-ai/main.go` | Binary entrypoint and version handoff. |
...
| `internal/components/opencodeplugin/` | Optional OpenCode TUI plugin registration, including external package names and the managed Hgtran Logo local plugin. |
...
| `internal/state/state.go` | Persisted install state in `~/.hgtran-ai/state.json`. |
...
| Engram store schema | Sessions, observations, prompts, relations, and sync mutations are external Engram runtime concepts, not hgtran-ai source files. |
